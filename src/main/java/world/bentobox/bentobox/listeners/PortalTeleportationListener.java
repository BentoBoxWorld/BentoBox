...
public class PortalTeleportationListener implements Listener {
  ...
  public void onPlayerPortal(PlayerMoveEvent e) {
    ...
    if (
      inPortal.contains(uuid) ||
      !plugin.getIWM().inWorld(Util.getWorld(e.getFrom().getWorld()))
    ) {
      return;
    }
    if (
    ...
    if (
      !Bukkit.getAllowEnd() &&
      e.getPlayer().getLocation().getBlock().getType().equals(Material.END_PORTAL)
    ) {
      PlayerPortalEvent en = new PlayerPortalEvent(
        e.getPlayer(),
        e.getPlayer().getLocation(),
  ...
  public boolean onEndIslandPortal(PlayerPortalEvent e) {
    ...
    if (!plugin.getIWM().isEndIslands(overWorld)) {
      ...
      // From standard end - check if player has an island to go to
        ...
        plugin.getIslands().hasIsland(overWorld, e.getPlayer().getUniqueId()) ||
    ...
    if (fromWorld.getEnvironment() == Environment.THE_END) {
      ...
      Location to = plugin
        .getIslands()
        .getIslandAt(e.getFrom())
        .map(i -> i.getSpawnPoint(Environment.NORMAL))
        .orElse(e.getFrom().toVector().toLocation(overWorld));
      e.setCancelled(true);
    ...
    Location to = optionalIsland
      .map(i -> i.getSpawnPoint(Environment.THE_END))
      .orElse(e.getFrom().toVector().toLocation(endWorld));
    e.setCancelled(true);
  ...
  public boolean onNetherPortal(PlayerPortalEvent e) {
    ...
    if (!plugin.getIWM().isNetherIslands(overWorld)) {
      ...
      // From standard nether
        ...
        plugin
          .getIslands()
          .homeTeleportAsync(overWorld, e.getPlayer())
          .thenAccept(b -> inPortal.remove(e.getPlayer().getUniqueId()));
      }
      return true;
    }
    // FROM NETHER
    ...
    Optional<Island> optionalIsland = plugin.getIslands().getIslandAt(e.getFrom());
    Location to = optionalIsland
      .map(i -> i.getSpawnPoint(Environment.NETHER))
      .orElse(e.getFrom().toVector().toLocation(nether));
    e.setCancelled(true);
    ...
    plugin
      ...
      .ifPresent(
        addon -> {
          ...
          BlueprintBundle bb = plugin
            .getBlueprintsManager()
            .getDefaultBlueprintBundle(addon);
          if (bb != null) {
            Blueprint bp = plugin
              .getBlueprintsManager()
              .getBlueprints(addon)
              .get(bb.getBlueprint(env));
            if (bp != null) {
              new BlueprintPaster(plugin, bp, to.getWorld(), island)
                .paste()
                .thenAccept(
                  b ->
                    new SafeSpotTeleport.Builder(plugin)
                      .entity(player)
                      .location(
                        island.getSpawnPoint(env) == null ? to : island.getSpawnPoint(env)
                      )
              ...
              plugin.logError(
                "Could not paste default island in nether or end. Is there a nether-island or end-island blueprint?"
              );
            }
          }
        }
