...
/**
 * Prevents visitors from losing their items if they die on an island in which
 * they are a visitor. Coops and above are not considered visitors. Handles
 * {@link world.bentobox.bentobox.lists.Flags#VISITOR_KEEP_INVENTORY}.
 * 
 * @author jstnf
 * @since 1.17.0
 */
public class VisitorKeepInventoryListener extends FlagListener {
    ...
    public void onVisitorDeath(PlayerDeathEvent e) {
        ...
        if (!getIWM().inWorld(world) || !Flags.VISITOR_KEEP_INVENTORY.isSetForWorld(world)) {
            ...
            this.report(User.getInstance(e.getEntity()), e, e.getEntity().getLocation(), Flags.VISITOR_KEEP_INVENTORY,
        ...
        if (island.isPresent()
                && !island.get().getMemberSet(RanksManager.COOP_RANK).contains(e.getEntity().getUniqueId())) {
