...
/**
 * Cleans super-flat world chunks or normal nether chunks if they generate
 * accidentally due to lack of a generator being loaded
 * 
 * @author tastybento
 */
public class CleanSuperFlatListener extends FlagListener {
    ...
    /**
     * Task that runs each tick to regenerate chunks that are in the
     * {@link #chunkQueue}. It regenerates them one at a time.
     * 
     * @since 1.1
     */
    @Nullable
    ...
    /**
     * Whether BentoBox is ready or not. This helps to avoid hanging out the server
     * on startup as a lot of {@link ChunkLoadEvent} are called at this time.
     * 
     * @since 1.1
     */
    ...
    public void onChunkLoad(ChunkLoadEvent e) {
        ...
        if (cg == null) {
            ...
            this.plugin
                    .logWarning("This is often caused by the 'use-own-generator' being set to 'true' in the gamemode's"
                            + " configuration while there hasn't been any custom world generator assigned to the world.");
            this.plugin.logWarning(
                    "Either revert the changes in the gamemode's config.yml or assign your custom world generator to the world.");
    ...
    private void cleanChunk(World world) {
        if (!this.chunkQueue.isEmpty()) {
            ...
            if (this.plugin.getSettings().isLogCleanSuperFlatChunks()) {
                this.plugin.log("Regenerating superflat chunk in " + world.getName() + " at (" + chunk.getX() + ", "
                        + chunk.getZ() + ") " + "(" + this.chunkQueue.size() + " chunk(s) remaining in the queue)");
            }
        } else {
            this.task.cancel();
        }
    ...
    private boolean noClean(World world, ChunkLoadEvent e) {
        ...
        if (!this.getIWM().inWorld(world) || !Flags.CLEAN_SUPER_FLAT.isSetForWorld(world)
                || world.getEnvironment().equals(Environment.NETHER) && (!this.plugin.getIWM().isNetherGenerate(world)
                        || !this.plugin.getIWM().isNetherIslands(world))
        ...
        // Due to flat super flat chunk generation changes in 1.19, they now are
        // generated properly at the world min.
