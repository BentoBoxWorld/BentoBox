...
public class CleanSuperFlatListener extends FlagListener {
  ...
  public void onChunkLoad(ChunkLoadEvent e) {
    ...
    ChunkGenerator cg = plugin
      .getAddonsManager()
      .getDefaultWorldGenerator(world.getName(), "");
    if (cg == null) {
      ...
      plugin.logWarning(
        "This is often caused by the 'use-own-generator' being set to 'true' in the gamemode's" +
    ...
    if (task == null || task.isCancelled()) {
      task =
        Bukkit
          .getScheduler()
          .runTaskTimer(plugin, () -> cleanChunk(e, world, cg, grid), 0L, 1L);
    }
  }
  ...
  private void cleanChunk(
    ...
    if (!chunkQueue.isEmpty()) {
      ...
      cg
        ...
        .forEach(pop -> pop.populate(world, random, e.getChunk()));
      if (plugin.getSettings().isLogCleanSuperFlatChunks()) {
        plugin.log(
          "Regenerating superflat chunk in " +
          world.getName() +
          " at (" +
          chunkXZ.x +
          ", " +
          ...
          "(" +
          chunkQueue.size() +
          " chunk(s) remaining in the queue)"
        );
      }
    } else {
      task.cancel();
    ...
    return (
      ...
      (
        ...
        (
          ...
          (
            !plugin.getIWM().isNetherGenerate(world) ||
