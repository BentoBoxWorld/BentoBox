...
public class LiquidsFlowingOutListener extends FlagListener {
    ...
    public void onLiquidFlow(BlockFromToEvent e) {
        ...
        // Time to do some maths! We've got the vector FromTo, let's check if its y
        // coordinate is different from zero.
        if (to.getLocation().toVector().subtract(from.getLocation().toVector()).getY() != 0) {
            // We do not run any checks if this is a vertical flow - would be too much
            // resource consuming.
            return;
        }

        // Only prevent if it is flowing into the area between islands or into another
        // island.
    ...
    /**
     * Prevents players from dispensing water, lava and powdered snow from dispenser
     * outside island if Flags.LIQUIDS_FLOWING_OUT is disabled.
     * 
