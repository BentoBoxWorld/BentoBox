...
public class LiquidsFlowingOutListener extends FlagListener {
  ...
  public void onLiquidFlow(BlockFromToEvent e) {
    ...
    if (
      !getIWM().inWorld(from.getLocation()) ||
      Flags.LIQUIDS_FLOWING_OUT.isSetForWorld(from.getWorld())
    ) {
    ...
    if (
      !toIsland.isPresent() || (fromIsland.isPresent() && !fromIsland.equals(toIsland))
    ) {
      e.setCancelled(true);
    }
  }
