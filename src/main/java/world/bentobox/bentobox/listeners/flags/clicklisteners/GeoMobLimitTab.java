...
public class GeoMobLimitTab implements Tab, ClickHandler {
   ...
   */
  private static final List<EntityType> LIVING_ENTITY_TYPES = Collections.unmodifiableList(
    Arrays
      ...
      .filter(
        t ->
          !(
            t.equals(EntityType.PLAYER) ||
            t.equals(EntityType.GIANT) ||
            t.equals(EntityType.ARMOR_STAND)
          )
      )
  ...
  public boolean onClick(Panel panel, User user, ClickType clickType, int slot) {
    ...
    plugin
      .getIWM()
      .getAddon(Util.getWorld(user.getWorld()))
  ...
  public String getName() {
    return type == EntityLimitTabType.MOB_LIMIT
      ? user.getTranslation("protection.flags.LIMIT_MOBS.name")
    ...
    if (type == EntityLimitTabType.MOB_LIMIT) {
      if (
        !BentoBox
          .getInstance()
          .getIWM()
          .getMobLimitSettings(user.getWorld())
          .contains(c.name())
      ) {
      ...
      if (
        BentoBox
          .getInstance()
          .getIWM()
          .getGeoLimitSettings(user.getWorld())
          .contains(c.name())
      ) {
        pib.icon(Material.GREEN_SHULKER_BOX);
