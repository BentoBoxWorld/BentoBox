...
public class ObsidianScoopingListener extends FlagListener {
  ...
  public boolean onPlayerInteract(final PlayerInteractEvent e) {
    if (
      !getIWM().inWorld(e.getPlayer().getLocation()) ||
    ...
    if (getIslands().userIsOnIsland(user.getWorld(), user)) {
      ...
      if (
        getBlocksAround(b)
          .stream()
          .anyMatch(block -> block.getType().equals(Material.OBSIDIAN))
      ) {
      ...
      e
        .getPlayer()
        .getWorld()
        .playSound(e.getPlayer().getLocation(), Sound.ITEM_BUCKET_FILL_LAVA, 1F, 1F);
