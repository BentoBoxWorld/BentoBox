...
/**
 * Handles interaction with beds Note - bed protection from breaking or placing
 * is done elsewhere.
 * 
 * @author tastybento
 *
...
public class BucketListener extends FlagListener {
    ...
    /**
     * Prevents collecting of lava, water, milk. If bucket use is denied in general,
     * it is blocked.
     * 
     * @param e - event
     */
    ...
    /**
     * Prevents collecting mushroom strew from MushroomCow if player does not have
     * access to Milking flag.
     * 
     * @param e - event
     */
    @EventHandler(priority = EventPriority.LOW, ignoreCancelled = true)
    public void onBowlFill(final PlayerInteractEntityEvent e) {
        if (e.getRightClicked() instanceof MushroomCow
                && e.getPlayer().getInventory().getItemInMainHand().getType().equals(Material.BOWL)) {
