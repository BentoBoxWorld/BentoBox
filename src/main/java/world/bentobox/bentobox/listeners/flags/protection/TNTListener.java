...
public class TNTListener extends FlagListener {
   ...
   * @since 1.5.0
   */
  private static final List<EntityType> TNT_TYPES = Collections.unmodifiableList(
    Arrays.asList(EntityType.PRIMED_TNT, EntityType.MINECART_TNT)
  );
   ...
   * @since 1.5.0
   */
  private static final List<Material> PRIMING_ITEMS = Collections.unmodifiableList(
    Arrays.asList(Material.FLINT_AND_STEEL, Material.FIRE_CHARGE)
  );
  ...
  public boolean onTNTDamage(EntityChangeBlockEvent e) {
    // Check world
    if (
      !e.getBlock().getType().equals(Material.TNT) ||
      !getIWM().inWorld(e.getBlock().getLocation())
    ) {
      return false;
    }
  ...
  protected boolean protect(Location location) {
    return getIslands()
      .getProtectedIslandAt(location)
      .map(i -> !i.isAllowed(Flags.TNT_DAMAGE))
