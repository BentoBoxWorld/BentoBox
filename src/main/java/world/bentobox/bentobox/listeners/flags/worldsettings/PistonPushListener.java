...
public class PistonPushListener extends FlagListener {
    ...
    public void onPistonExtend(BlockPistonExtendEvent e) {
        ...
        if (Flags.PISTON_PUSH.isSetForWorld(e.getBlock().getWorld())) {
            getIslands().getProtectedIslandAt(e.getBlock().getLocation()).ifPresent(i -> e.setCancelled(
                    // Run through the location of all the relative blocks and see if they are
                    // outside the island
                    !e.getBlocks().stream().map(b -> b.getRelative(e.getDirection()).getLocation())
