...
public class MobTeleportListener extends FlagListener {
    ...
    public void onEntityTeleportEvent(EntityTeleportEvent event) {
        ...
        // If not in the right world exit immediately and check teleportation is within
        // the same world
        if (!this.getIWM().inWorld(w) || event.getTo() == null
        ...
        if (event.getEntityType().equals(EntityType.ENDERMAN)
                && Boolean.TRUE.equals(island.map(i -> !i.isAllowed(Flags.ENDERMAN_TELEPORT))
                        .orElseGet(() -> !Flags.ENDERMAN_TELEPORT.isSetForWorld(w)))) {
            // Enderman teleport is disabled on island or world. Cancel it.
            event.setCancelled(true);
        }

        if (event.getEntityType().equals(EntityType.SHULKER)
                && Boolean.TRUE.equals(island.map(i -> !i.isAllowed(Flags.SHULKER_TELEPORT))
                        .orElseGet(() -> !Flags.SHULKER_TELEPORT.isSetForWorld(w)))) {
