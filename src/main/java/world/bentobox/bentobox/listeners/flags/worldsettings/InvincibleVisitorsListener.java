...
public class InvincibleVisitorsListener extends FlagListener implements ClickHandler {
  ...
  public boolean onClick(Panel panel, User user, ClickType clickType, int slot) {
    ...
    String reqPerm =
      getIWM().getPermissionPrefix(Util.getWorld(user.getWorld())) +
      "admin.settings.INVINCIBLE_VISITORS";
    ...
    if (panel.getName().equals(ivPanelName)) {
      ...
      DamageCause c = Arrays
        .stream(EntityDamageEvent.DamageCause.values())
        .sorted(Comparator.comparing(DamageCause::name))
  ...
  private void openPanel(User user, String ivPanelName) {
    ...
    Arrays
      .stream(EntityDamageEvent.DamageCause.values())
      .sorted(Comparator.comparing(DamageCause::name))
      .forEach(c -> pb.item(getPanelItem(c, user)));
    pb.build();
  }
