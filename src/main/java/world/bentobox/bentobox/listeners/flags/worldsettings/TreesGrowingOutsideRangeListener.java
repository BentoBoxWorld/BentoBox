...
public class TreesGrowingOutsideRangeListener extends FlagListener {
  ...
  public void onTreeGrow(StructureGrowEvent e) {
    if (
      !getIWM().inWorld(e.getWorld()) ||
      Flags.TREES_GROWING_OUTSIDE_RANGE.isSetForWorld(e.getWorld())
    ) {
    ...
    e
      ...
      .filter(
        blockState ->
          !getIslands().getProtectedIslandAt(blockState.getLocation()).isPresent()
      )
  ...
  public void onChorusGrow(BlockSpreadEvent e) {
    if (
      !getIWM().inWorld(e.getSource().getWorld()) ||
