...
public class MobSpawnListener extends FlagListener {
    ...
    public void onRaidStartEvent(RaidTriggerEvent event) {
        ...
        if (Boolean.TRUE.equals(island.map(i -> !i.isAllowed(Flags.MONSTER_NATURAL_SPAWN))
                .orElseGet(() -> !Flags.MONSTER_NATURAL_SPAWN.isSetForWorld(event.getWorld())))) {
    ...
    void onMobSpawn(CreatureSpawnEvent e) {
        ...
        case DEFAULT, DROWNED, JOCKEY, LIGHTNING, MOUNT, NATURAL, NETHER_PORTAL, OCELOT_BABY, PATROL, RAID,
            ...
            boolean cancelNatural = this.shouldCancel(e.getEntity(), e.getLocation(), Flags.ANIMAL_NATURAL_SPAWN,
                    Flags.MONSTER_NATURAL_SPAWN);
            e.setCancelled(cancelNatural);
        }
        // Spawners
        case SPAWNER -> {
            boolean cancelSpawners = this.shouldCancel(e.getEntity(), e.getLocation(), Flags.ANIMAL_SPAWNERS_SPAWN,
    ...
    /**
     * This method checks if entity should be cancelled from spawning in given
     * location base on flag values.
     * 
     ...
     * @return {@code true} if flag prevents entity to spawn, {@code false}
     *         otherwise.
     */
