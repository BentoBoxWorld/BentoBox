...
public class MobSpawnListener extends FlagListener {
  ...
  public boolean onMobSpawn(CreatureSpawnEvent e) {
    ...
    switch (e.getSpawnReason()) {
      ...
      case VILLAGE_INVASION:
        boolean cancelNatural = shouldCancel(
          e.getEntity(),
          e.getLocation(),
      ...
      // Spawners
      case SPAWNER:
        boolean cancelSpawners = shouldCancel(
          e.getEntity(),
          e.getLocation(),
  ...
  private boolean shouldCancel(
    Entity entity,
    Location loc,
    Flag animalSpawnFlag,
    Flag monsterSpawnFlag
  ) {
    Optional<Island> island = getIslands().getIslandAt(loc);
    if (Util.isHostileEntity(entity) && !(entity instanceof PufferFish)) {
      return island
        .map(i -> !i.isAllowed(monsterSpawnFlag))
        .orElse(!monsterSpawnFlag.isSetForWorld(entity.getWorld()));
    } else if (Util.isPassiveEntity(entity) || entity instanceof PufferFish) {
      return island
        .map(i -> !i.isAllowed(animalSpawnFlag))
        .orElse(!animalSpawnFlag.isSetForWorld(entity.getWorld()));
    }
    return false;
  }
}
