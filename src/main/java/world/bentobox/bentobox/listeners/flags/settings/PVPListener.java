...
public class PVPListener extends FlagListener {
  ...
  public void onEntityDamage(EntityDamageByEntityEvent e) {
    if (
      ...
      if (
        ...
        protectedVisitor((Player) e.getEntity())
      ) {
        if (e.getDamager() instanceof Player) {
          User
            .getInstance(e.getDamager())
            .notify(Flags.INVINCIBLE_VISITORS.getHintReference());
        } else if (
          e.getDamager() instanceof Projectile &&
          ((Projectile) e.getDamager()).getShooter() instanceof Player
        ) {
          User
            .getInstance((Player) ((Projectile) e.getDamager()).getShooter())
            .notify(Flags.INVINCIBLE_VISITORS.getHintReference());
        }
        e.setCancelled(true);
      } else {
    ...
    if (
      ...
      if (protectedVisitor((Player) e.getCaught())) {
        ...
        !checkIsland(
          e,
          e.getPlayer(),
          e.getCaught().getLocation(),
          getFlag(e.getCaught().getWorld())
        )
      ) {
        e.getHook().remove();
        User
          .getInstance(e.getPlayer())
          .notify(getFlag(e.getCaught().getWorld()).getHintReference());
        e.setCancelled(true);
      }
    }
  }
  ...
  public void onSplashPotionSplash(final PotionSplashEvent e) {
    if (
      ...
      for (LivingEntity le : e.getAffectedEntities()) {
        if (
          !le.getUniqueId().equals(user.getUniqueId()) &&
  ...
  private boolean protectedVisitor(LivingEntity entity) {
    return (
      getPlugin()
        .getIWM()
        .getIvSettings(entity.getWorld())
        .contains(DamageCause.ENTITY_ATTACK.name()) &&
  ...
  public void onLingeringPotionSplash(final LingeringPotionSplashEvent e) {
    ...
    if (
      ...
      thrownPotions.put(
        e.getAreaEffectCloud().getEntityId(),
  ...
  public void onPVPFlagToggle(final FlagSettingChangeEvent e) {
    ...
    if (
      Flags.PVP_OVERWORLD.equals(flag) ||
      Flags.PVP_NETHER.equals(flag) ||
      Flags.PVP_END.equals(flag)
    ) {
      String message =
        "protection.flags." + flag.getID() + "." + (e.isSetTo() ? "enabled" : "disabled");
      // Send the message to visitors
      e
        .getIsland()
        .getVisitors()
        .forEach(visitor -> User.getInstance(visitor).sendMessage(message));
      // Send the message to island members (and coops and trusted)
      e
        .getIsland()
        .getMemberSet(RanksManager.COOP_RANK)
        .forEach(member -> User.getInstance(member).sendMessage(message));
    }
  }
}
