...
public class WitherListener extends FlagListener {
    ...
    /**
     * Withers change blocks to air after they are hit (don't know why) This
     * prevents this when the wither has been spawned
     * 
     * @param e - event
     */
    @EventHandler(priority = EventPriority.LOW, ignoreCancelled = true)
    public void onWitherChangeBlocks(final EntityChangeBlockEvent e) {
        if (!getIWM().inWorld(e.getBlock().getWorld()))
            return;
        e.setCancelled(e.getEntityType().equals(EntityType.WITHER)
                && !Flags.WITHER_DAMAGE.isSetForWorld(e.getBlock().getWorld()));
    }
}
