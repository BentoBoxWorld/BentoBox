...
public class BreakBlocksListener extends FlagListener {
  ...
  public void onPlayerInteract(final PlayerInteractEvent e) {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (
          lastBlock.getType().toString().endsWith("_SKULL") ||
          (
            lastBlock.getType().toString().endsWith("_HEAD") &&
            !lastBlock.getType().equals(Material.PISTON_HEAD)
          )
        ) {
  ...
  public void onVehicleDamageEvent(VehicleDamageEvent e) {
    if (
      getIWM().inWorld(e.getVehicle().getLocation()) && e.getAttacker() instanceof Player
    ) {
  ...
  public void onEntityDamage(EntityDamageByEntityEvent e) {
    ...
    if (e.getDamager() instanceof Player) {
      ...
      if (
        ...
        notAllowed(e, (Player) p.getShooter(), e.getEntity().getLocation())
      ) {
  ...
  public void onProjectileHitBreakBlock(ProjectileHitEvent e) {
    ...
    if (
      e.getHitBlock() == null || !e.getHitBlock().getType().equals(Material.CHORUS_FLOWER)
    ) {
      return;
    }
    ...
    if (
      ...
      getPlugin()
        .getServer()
        .getScheduler()
        .runTask(getPlugin(), () -> e.getHitBlock().setBlockData(data, true));
