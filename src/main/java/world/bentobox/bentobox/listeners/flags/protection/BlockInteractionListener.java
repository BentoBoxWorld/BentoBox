...
public class BlockInteractionListener extends FlagListener {
    ...
    private boolean checkSpecialCases(Event e, Player player, Block block) {
        ...
        if (block.getState() instanceof BrushableBlock && BlockInteractionListener.holds(player, Material.BRUSH)) {
            // Protect this using break blocks flag for now. Maybe in the future it can have
            // its own flag.
    ...
    /**
     * When breaking blocks is allowed, this protects specific blocks from being
     * broken, which would bypass the protection. For example, player enables break
     * blocks, but chests are still protected Fires after the BreakBlocks check.
     *
     * @param e - event
     */
    @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
    ...
    public void onDragonEggTeleport(BlockFromToEvent e) {
        ...
        // This protects against eggs dropping into adjacent islands, e.g. island
        // distance and protection range are equal
