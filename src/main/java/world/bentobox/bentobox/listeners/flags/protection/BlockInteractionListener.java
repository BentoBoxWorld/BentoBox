...
public class BlockInteractionListener extends FlagListener {
    ...
    switch (type) {
      ...
      default:
        if (stringFlags.containsKey(type.name())) {
          Optional<Flag> f = BentoBox
            .getInstance()
            .getFlagsManager()
            .getFlag(stringFlags.get(type.name()));
  ...
  public void onDragonEggTeleport(BlockFromToEvent e) {
    ...
    if (
      ...
      !getIWM().inWorld(block.getLocation())
    ) {
      return;
    }
    ...
    Optional<Island> toIsland = getIslands()
      .getProtectedIslandAt(e.getToBlock().getLocation());
    fromIsland.ifPresent(
      from -> e.setCancelled(toIsland.map(to -> to != from).orElse(true))
    );
  }
}
