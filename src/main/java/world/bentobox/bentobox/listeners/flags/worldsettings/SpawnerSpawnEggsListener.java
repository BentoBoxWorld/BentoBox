...
public class SpawnerSpawnEggsListener extends FlagListener {
    ...
    public void onSpawnerChange(final PlayerInteractEvent e) {
        ...
        if (e.getClickedBlock() != null && e.getClickedBlock().getType().equals(Material.SPAWNER) && e.getItem() != null
                && e.getItem().getType().toString().endsWith("_SPAWN_EGG")
                && getIWM().inWorld(e.getClickedBlock().getWorld())
                && !(user
                        .hasPermission(getIWM().getPermissionPrefix(e.getClickedBlock().getWorld()) + "mod.bypass."
                                + Flags.SPAWNER_SPAWN_EGGS.getID() + ".everywhere")
                        || user.hasPermission(
                                getIWM().getPermissionPrefix(e.getClickedBlock().getWorld()) + "mod.bypassprotect"))
            ...
            user.notify("protection.protected", TextVariables.DESCRIPTION,
                    user.getTranslation(Flags.SPAWNER_SPAWN_EGGS.getHintReference()));
        }
    }
}
