...
public class CommandRankClickListener implements ClickHandler {
  ...
  public boolean onClick(Panel panel, User user, ClickType clickType, int slot) {
    ...
    if (
      !user.hasPermission(reqPerm) &&
      !user.hasPermission(allPerms) &&
      !user.isOp() &&
      ...
      .forEach(
        (reference, score) -> {
          if (score >= RanksManager.MEMBER_RANK && score < island.getRankCommand(c)) {
            pib.description(
              user.getTranslation("protection.panel.flag-item.blocked-rank") +
              user.getTranslation(reference)
            );
          } else if (
            score <= RanksManager.OWNER_RANK && score > island.getRankCommand(c)
          ) {
            pib.description(
              user.getTranslation("protection.panel.flag-item.allowed-rank") +
              user.getTranslation(reference)
            );
          } else if (score == island.getRankCommand(c)) {
            pib.description(
              user.getTranslation("protection.panel.flag-item.minimal-rank") +
              user.getTranslation(reference)
            );
          }
        }
  ...
  private List<String> getCmdRecursively(String labels, CompositeCommand cc) {
    ...
    cc
      .getSubCommands()
      .values()
      .forEach(s -> result.addAll(getCmdRecursively(newLabel + " ", s)));
    return result;
  }
}
