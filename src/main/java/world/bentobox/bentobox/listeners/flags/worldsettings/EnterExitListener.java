...
public class EnterExitListener extends FlagListener {
  ...
  private static final String ISLAND_MESSAGE =
  ...
  private void sendExitNotification(@NonNull User user, @NonNull Island island) {
    ...
    if (island.isOwned()) {
      ...
      if (island.getMemberSet().contains(user.getUniqueId())) {
        user.notify(
          island.getWorld(),
          "protection.flags.ENTER_EXIT_MESSAGES.now-leaving-your-island",
          TextVariables.NAME,
          (island.getName() != null)
            ? island.getName()
            : user.getTranslation(
  ...
  private void sendEnterNotification(@NonNull User user, @NonNull Island island) {
    ...
    if (island.isOwned()) {
      ...
      if (island.getMemberSet().contains(user.getUniqueId())) {
        user.notify(
          island.getWorld(),
          "protection.flags.ENTER_EXIT_MESSAGES.now-entering-your-island",
          TextVariables.NAME,
          (island.getName() != null)
            ? island.getName()
            : user.getTranslation(
