...
public class JoinLeaveListener implements Listener {
  ...
  public void onPlayerJoin(final PlayerJoinEvent event) {
    ...
    if (!user.getName().isEmpty()) {
      ...
      plugin.logWarning(
        "Player that just logged in has no name! " + playerUUID.toString()
      );
    }

    // If mobs have to be removed when a player joins, then wipe all the mobs on his island.
    if (
      plugin.getIslands().locationIsOnIsland(event.getPlayer(), user.getLocation()) &&
      Flags.REMOVE_MOBS.isSetForWorld(user.getWorld())
    ) {
      Bukkit
        .getScheduler()
        .runTask(plugin, () -> plugin.getIslands().clearArea(user.getLocation()));
    }
        ...
        w -> {
          ...
          if (
            ...
            Runnable createIsland = () -> {
              ...
              if (
                !plugin.getIWM().isCreateIslandOnFirstLoginAbortOnLogout(w) ||
                user.isOnline()
              ) {
                plugin
                  .getIWM()
                          ...
                          command ->
                            command.execute(
                              ...
                              Collections.singletonList(
                                BlueprintsManager.DEFAULT_BUNDLE_NAME
                              )
                            )
                        )
                  );
  ...
  private void runAutomatedOwnershipTransfer(User user) {
    plugin
      ...
      .filter(
        world ->
          plugin.getIslands().hasIsland(world, user) &&
          !plugin.getIslands().isOwner(world, user.getUniqueId())
      )
      .forEach(
        world -> {
          ...
          long inactivityThreshold =
            plugin.getSettings().getAutoOwnershipTransferInactivityThreshold() *
            24 *
            60 *
            60 *
            1000L;
          ...
          if (owner.getLastPlayed() != 0 && owner.getLastPlayed() < timestamp) {
            ...
            List<UUID> candidates = Arrays.asList(
              (UUID[]) island
                .getMemberSet()
                .stream()
                ...
                .filter(
                  uuid ->
                    Bukkit.getOfflinePlayer(uuid).getLastPlayed() != 0 &&
            ...
            if (
              !candidates.isEmpty() &&
              !plugin.getSettings().isAutoOwnershipTransferIgnoreRanks()
            ) {
      ...
      .forEach(
        world -> {
          ...
          if (island != null) {
            ...
            int range = user.getPermissionValue(
              plugin
                .getIWM()
            ...
            if (range != island.getProtectionRange()) {
              ...
              plugin.log(
                "Island protection range changed from " +
                island.getProtectionRange() +
                " to " +
                range +
                " for " +
  ...
  public void onPlayerQuit(final PlayerQuitEvent event) {
      ...
      .forEach(
        w -> {
          Island island = plugin
            .getIslands()
            .getIsland(w, User.getInstance(event.getPlayer()));
            ...
            Bukkit
              ...
              .noneMatch(
                p ->
                  plugin
                    .getIslands()
                    .getMembers(w, event.getPlayer().getUniqueId())
              ...
              .forEach(
                e ->
                  User
                    ...
                    .sendMessage(
                      "commands.island.team.uncoop.all-members-logged-off",
                      TextVariables.NAME,
    ...
    plugin
      .getIslands()
      .clearRank(RanksManager.COOP_RANK, event.getPlayer().getUniqueId());
