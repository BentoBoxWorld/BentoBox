...
public class EntityTeleportListener extends AbstractTeleportListener implements Listener {
    ...
    /**
     * This listener checks entity portal events and triggers appropriate methods to
     * transfer entities to the correct location in other dimension.
     * <p>
     * This event is triggered when entity is about to being teleported because of
    ...
    public void onEntityEnterPortal(EntityPortalEnterEvent event) {
        ...
        if (!Bukkit.getAllowNether() && type.equals(Material.NETHER_PORTAL)) {
            this.portalProcess(new EntityPortalEvent(entity, event.getLocation(), event.getLocation(), 0),
...
// ---------------------------------------------------------------------
    ...
    private void portalProcess(EntityPortalEvent event, World.Environment environment) {
        ...
        // it's the vector
        event.setTo(this.calculateLocation(event.getFrom(), fromWorld, toWorld, environment,
        ...
        if (this.isPastingMissingIslands(overWorld) && this.isAllowedInConfig(overWorld, environment)
            ...
            // If there is no island, then processor already entity cannot be teleported
            // before player
        ...
        Bukkit.getScheduler().runTask(this.plugin, () -> {
            if (!event.getEntity().getWorld().equals(toWorld)) {
                ...
                ClosestSafeSpotTeleport.builder(this.plugin).entity(event.getEntity()).location(event.getTo()).portal()
                        .successRunnable(() -> {
    ...
    private void handleToStandardNetherOrEnd(EntityPortalEvent event, World overWorld, World.Environment environment) {
        ...
        if (World.Environment.THE_END.equals(toWorld.getEnvironment()) && spawnPoint.getBlockX() == 0
        ...
        if (this.isAllowedOnServer(environment)) {
            ...
            ClosestSafeSpotTeleport.builder(this.plugin).entity(event.getEntity()).location(spawnPoint).portal()
                    .build();
        }
    }
     ...
     */
    private void handleFromStandardNetherOrEnd(EntityPortalEvent event, World overWorld,
