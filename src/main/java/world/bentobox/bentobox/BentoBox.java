...
public class BentoBox extends JavaPlugin {
  ...
  public void onEnable() {
    if (!ServerCompatibility.getInstance().checkCompatibility().isCanLaunch()) {
      ...
      List<String> versions = ServerCompatibility
        .ServerVersion.getVersions(
          ServerCompatibility.Compatibility.COMPATIBLE,
          ServerCompatibility.Compatibility.SUPPORTED
        )
        .stream()
        .map(ServerCompatibility.ServerVersion::toString)
  ...
  private void completeSetup(long loadTime) {
    ...
    addonsManager
      .getGameModeAddons()
      .forEach(placeholdersManager::registerDefaultPlaceholders);
    ...
    Bukkit
      ...
      .runTaskTimer(
        ...
        getSettings().getDatabaseBackupPeriod() * 20 * 60L,
        getSettings().getDatabaseBackupPeriod() * 20 * 60L
      );
  ...
  public boolean loadSettings() {
    ...
    if (settings == null) {
      ...
      logError(
        "Settings did not load correctly - disabling plugin - please check config.yml"
      );
