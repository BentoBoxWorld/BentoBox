...
public class ManagementPanel {
  ...
  private static final int[] PANES = {
    0,
    4,
    5,
    8,
    ...
    for (int i : PANES) {
      builder.item(
        ...
        new PanelItemBuilder()
          .icon(plugin.getSettings().getPanelFillerMaterial())
          .name(" ")
          .build()
      );
    }
      ...
      case GAMEMODES:
        ...
        for (Addon addon : addons) {
          ...
          PanelItem addonItem = new PanelItemBuilder()
            ...
            .name(
              user.getTranslation(
                LOCALE_REF + "views.gamemodes.gamemode.name",
                TextVariables.NAME,
            ...
            .description(
              user.getTranslation(
                LOCALE_REF + "views.gamemodes.gamemode.description",
                "[islands]",
                String.valueOf(
                  addon.getIslands().getIslandCount(gameModeAddon.getOverWorld())
                )
              )
            )
            .clickHandler(
          ...
          PanelItem blueprints = new PanelItemBuilder()
            ...
            .description(
              user.getTranslation(LOCALE_REF + "views.gamemodes.blueprints.description")
            )
            .clickHandler(
              (panel, user1, clickType, slot) -> {
  ...
  private static void setupHeader(PanelBuilder builder, User user, View view) {
    ...
    // BentoBox state icon
    ServerCompatibility.Compatibility compatibility = ServerCompatibility
      .getInstance()
      .checkCompatibility();
    ServerCompatibility.ServerSoftware serverSoftware = ServerCompatibility
      .getInstance()
      .getServerSoftware();
    ServerCompatibility.ServerVersion serverVersion = ServerCompatibility
      .getInstance()
      .getServerVersion();
    ...
    PanelItemBuilder compatibilityItemBuilder = new PanelItemBuilder()
      ...
      .description(
        user.getTranslation(
          LOCALE_REF + "information.state.description." + compatibility,
          TextVariables.NAME,
          serverSoftware.equals(ServerSoftware.UNKNOWN)
            ? serverSoftware.getName()
            : serverSoftware.toString(),
          TextVariables.VERSION,
          serverVersion != null
            ? serverVersion.toString()
            : user.getTranslation("general.invalid")
        )
      );
