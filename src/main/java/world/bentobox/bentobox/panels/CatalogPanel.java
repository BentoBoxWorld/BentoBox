    ...
    if (catalog.isEmpty()) {
      ...
      for (CatalogEntry addon : catalog) {
        ...
        itemBuilder.description(
          user.getTranslation(
            LOCALE_REF + "icon.description-template",
            "[topic]",
            StringUtils.capitalize(
              user.getTranslation("catalog.topics." + addon.getTopic())
            ),
            "[install]",
            install,
            "[description]",
        ...
        itemBuilder.clickHandler(
          (panel, user1, clickType, slot) -> {
            user1.sendRawMessage(
              ChatColor.GRAY +
              "" +
              ChatColor.ITALIC +
              "https://github.com/" +
