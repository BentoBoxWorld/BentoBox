...
public class LanguagePanel {
  ...
  public static void openPanel(User user) {
    ...
    for (Locale locale : localesManager.getAvailableLocales(true)) {
      ...
      localeIcon
        .name(
          ChatColor.WHITE + WordUtils.capitalize(locale.getDisplayName(user.getLocale()))
        )
        .clickHandler(
          (panel, u, click, slot) -> {
            BentoBox
              .getInstance()
              .getPlayers()
              .setLocale(u.getUniqueId(), locale.toLanguageTag());
            u.sendMessage(
              "language.edited",
              "[lang]",
      ...
      if (user.getLocale().equals(locale)) {
        ...
        localeIcon.description(
          user.getTranslation("language.description.click-to-select"),
          ""
        );
      }
      ...
      for (String author : language.getAuthors()) {
        localeIcon.description(
          user.getTranslation("language.description.author", TextVariables.NAME, author)
        );
      }
