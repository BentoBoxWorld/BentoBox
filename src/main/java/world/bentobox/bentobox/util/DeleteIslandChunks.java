...
public class DeleteIslandChunks {
        ...
        .runTaskTimer(
          ...
          () -> {
            ...
            for (int i = 0; i < plugin.getSettings().getDeleteSpeed(); i++) {
              plugin
                .getIWM()
                .getAddon(di.getWorld())
                .ifPresent(
                  gm ->
                    // Overworld
                    processChunk(gm, Environment.NORMAL, chunkX, chunkZ)
                      .thenRun(
                        () ->
                          // Nether
                          processChunk(gm, Environment.NETHER, chunkX, chunkZ)
                            .thenRun(
                              () ->
                                // End
                                processChunk(gm, Environment.NETHER, chunkX, chunkZ)
  ...
  private CompletableFuture<Boolean> processChunk(
    GameModeAddon gm,
    Environment env,
    int x,
    int z
    ...
    switch (env) {
      case NETHER:
        // Nether
        if (
          plugin.getIWM().isNetherGenerate(di.getWorld()) &&
          plugin.getIWM().isNetherIslands(di.getWorld())
        ) {
      ...
      case THE_END:
        // End
        if (
          plugin.getIWM().isEndGenerate(di.getWorld()) &&
          plugin.getIWM().isEndIslands(di.getWorld())
        ) {
    ...
    if (PaperLib.isChunkGenerated(world, x, z)) {
      PaperLib
        .getChunkAtAsync(world, x, z)
        .thenAccept(chunk -> regenerateChunk(gm, chunk));
  ...
  private void regenerateChunk(GameModeAddon gm, Chunk chunk) {
    ...
    Arrays
      .stream(chunk.getTileEntities())
      .filter(te -> (te instanceof InventoryHolder))
      .filter(
        te -> di.inBounds(te.getLocation().getBlockX(), te.getLocation().getBlockZ())
      )
