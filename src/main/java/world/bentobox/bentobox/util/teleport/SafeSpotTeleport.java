...
public class SafeSpotTeleport {
  ...
  private void tryTogo(String failureMessage) {
    ...
    task =
      Bukkit
        .getScheduler()
        .runTaskTimer(plugin, () -> gatherChunks(failureMessage), 0L, SPEED);
  }
  ...
  private void gatherChunks(String failureMessage) {
    ...
    while (it.hasNext() && chunkSnapshot.size() < MAX_CHUNKS) {
      ...
      if (location.getWorld() != null) {
        ...
        chunkSnapshot.add(
          location.getWorld().getChunkAt(pair.x, pair.z).getChunkSnapshot()
        );
        if (!isLoaded) {
          ...
          () -> {
            ...
            if (!plugin.getIWM().inWorld(entity.getLocation())) {
              ...
              if (location.getWorld().getEnvironment().equals(Environment.NETHER)) {
                ...
                location.getWorld().getEnvironment().equals(Environment.THE_END)
              ) {
  ...
  private void makeAndTelport(Material m) {
    ...
    location
      ...
      .setType(m, false);
    Util
      .teleportAsync(entity, location.clone().add(new Vector(0.5D, 0D, 0.5D)))
      .thenRun(
        () -> {
          if (runnable != null) Bukkit.getScheduler().runTask(plugin, runnable);
          result.complete(true);
  ...
  private List<Pair<Integer, Integer>> getChunksToScan() {
    ...
    int maxRadius = plugin
      .getIslands()
      .getIslandAt(location)
      .map(Island::getProtectionRange)
      .orElse(plugin.getIWM().getIslandProtectionRange(location.getWorld()));
  ...
  private void addChunk(
    List<Pair<Integer, Integer>> chunksToScan,
    Pair<Integer, Integer> blockCoord,
    Pair<Integer, Integer> chunkCoord
  ) {
    if (
      !chunksToScan.contains(chunkCoord) &&
      plugin
        .getIslands()
        .getIslandAt(location)
        .map(is -> is.inIslandSpace(blockCoord))
        .orElse(true)
    ) {
      ...
      .runTask(
        ...
        () -> {
          if (!portal && entity instanceof Player && homeNumber > 0) {
            ...
            plugin
              .getPlayers()
              .setHomeLocation(User.getInstance(entity), loc, homeNumber);
          }
          Util
  ...
  boolean checkBlock(ChunkSnapshot chunk, int x, int y, int z) {
    ...
    Material space1 = chunk.getBlockType(
      x,
      Math.min(y + 1, SafeSpotTeleport.MAX_HEIGHT),
      z
    );
    Material space2 = chunk.getBlockType(
      x,
      Math.min(y + 2, SafeSpotTeleport.MAX_HEIGHT),
      z
    );
