...
public class ClosestSafeSpotTeleport {
    ...
    /**
     * This is main method that triggers safe spot search. It starts with the given
     * location and afterwards checks all blocks in required area.
     */
    void checkLocation() {
        ...
        // Players should not be teleported outside protection range if they already are
        // in it.
        this.boundingBox = this.plugin.getIslandsManager().getIslandAt(this.location)
                .map(Island::getProtectionBoundingBox).orElseGet(() -> {
                    ...
                    return new BoundingBox(this.location.getBlockX() - protectionRange,
                            ...
                            this.location.getBlockZ() - protectionRange, this.location.getBlockX() + protectionRange,
        ...
        this.range = Math.min(this.plugin.getSettings().getSafeSpotSearchRange(),
                (int) this.boundingBox.getWidthX() / 2);

        // The block queue contains all possible positions where player can be
        // teleported. The queue will not be populated
    ...
    /**
     * This method loads all chunks in async and populates blockQueue with all
     * blocks.
     */
    void gatherChunks() {
        ...
        if (!this.portal && !this.blockQueue.isEmpty() && this.blockQueue.peek().distance() < 5) {
            // Position is found? Well most likely (not in all situations) position in block
            // queue is already
            // the best position. The only bad situations could happen if position is on
            // chunk borders.
            this.finishTask();
            return;
        }
        ...
        Util.getChunkAtAsync(this.world, chunkPair.x, chunkPair.z).thenApply(Chunk::getChunkSnapshot)
    ...
    List<Pair<Integer, Integer>> getChunksToScan() {
        ...
        // Normalize block coordinates to chunk coordinates and add extra 1 for
        // visiting.
        int numberOfChunks = (((x + SCAN_RANGE) >> 4) - ((x - SCAN_RANGE) >> 4) + 1)
                * (((z + SCAN_RANGE) >> 4) - ((z - SCAN_RANGE) >> 4) + 1);

        // Ideally it would be if visitor switch from clockwise to counter-clockwise if
        // X % 16 < 8 and
    ...
    /**
     * This method adds chunk coordinates to the given chunksToScan list. The
     * limitation is that if location is in island, then block coordinate must also
     * be in island space.
     * 
     ...
     */
    void addChunk(List<Pair<Integer, Integer>> chunksToScan, Pair<Integer, Integer> blockCoord,
            Pair<Integer, Integer> chunkCoord) {
        if (!chunksToScan.contains(chunkCoord) && this.plugin.getIslandsManager().getIslandAt(this.location)
    ...
    /**
     * This method populates block queue with all blocks that player can be
     * teleported to. Add only positions that are inside BoundingBox and is safe for
     * teleportation.
     * 
    ...
    void scanAndPopulateBlockQueue(ChunkSnapshot chunkSnapshot) {
        ...
        Vector blockVector = new Vector(this.location.getBlockX(), this.location.getBlockY(),
            ...
            for (int z = 0; z < 16; z++) {
                ...
                for (int y = Math.max(minY, startY - this.range); y < Math.min(maxY, startY + this.range); y++) {
                    ...
                    if (this.boundingBox.contains(positionVector)) {
                        ...
                        PositionData positionData = new PositionData(positionVector,
                        ...
                        if (this.plugin.getIslandsManager().checkIfSafe(this.world, positionData.block,
                                positionData.spaceOne, positionData.spaceTwo)) {
    ...
    /**
     * This method finishes the chunk loading task and checks from all remaining
     * positions in block queue to find the best location for teleportation.
     * <p>
    ...
    /**
     * This method creates a spot in start location for player to be teleported to.
     * It creates 2 base material blocks above location and fills the space between
     * them with air.
     * 
    ...
    /**
     * This method scans all populated positions and returns true if position is
    ...
    /**
     * This method checks if given position is valid for teleportation. If query
     * should find portal, then it marks first best position as noPortalPosition and
     * continues to search for a valid portal. If query is not in portal mode, then
     * return first valid position.
     * 
     * @param positionData Position data that must be checked.
     * @return {@code true} if position is found and no extra processing required,
    ...
    boolean checkPosition(PositionData positionData) {
        if (this.portal) {
            if (Material.NETHER_PORTAL.equals(positionData.spaceOne())
                ...
                this.teleportEntity(new Location(this.world, positionData.vector().getBlockX() + 0.5,
                        positionData.vector().getBlockY() + 0.1, positionData.vector().getBlockZ() + 0.5,
                ...
                this.noPortalPosition = new Location(this.world, positionData.vector().getBlockX() + 0.5,
                        positionData.vector().getBlockY() + 0.1, positionData.vector().getBlockZ() + 0.5,
                        this.location.getYaw(), this.location.getPitch());
            }
        } else {
    ...
    /**
     * PositionData record holds information about position where player will be
     * teleported.
     * 
    ...
    public static class Builder {
        ...
        /**
         * Boolean that indicates if failing teleport should cancel it or create spot
         * for player.
         */
    ...
    /**
     * This comparator sorts position data based in order: - the smallest distance
     * value - the smallest x value - the smallest z value - the smallest y value
     */
    ...
    /**
     * The distance from starting location in all directions where new position will
     * be searched.
     */
    private int range;
    ...
    /**
     * This method returns first best available spot if portal was not found in
     * search area.
     */
