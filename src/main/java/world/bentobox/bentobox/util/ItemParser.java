...
public class ItemParser {
  ...
  private static ItemStack two(String[] part) {
    ...
    Material reqItem = Material.getMaterial(
      part[0].toUpperCase(java.util.Locale.ENGLISH)
    );
    if (reqItem == null) {
      return null;
    }
      ...
      if (part.length >= 2) {
        ...
        if (bannerMat == null) {
          BentoBox
            .getInstance()
            .logError(
              "Could not parse banner item " + part[0] + " so using a white banner."
            );
        ...
        if (meta != null) {
          for (int i = 2; i < part.length; i += 2) {
            meta.addPattern(
              new Pattern(DyeColor.valueOf(part[i + 1]), PatternType.valueOf(part[i]))
            );
          }
