...
public class Util {
  ...
  public static Location getClosestIsland(Location location) {
    ...
    long x =
      Math.round((double) location.getBlockX() / dist) *
      dist +
      plugin.getIWM().getIslandXOffset(location.getWorld());
    long z =
      Math.round((double) location.getBlockZ() / dist) *
      dist +
  ...
  public static String getStringLocation(final Location l) {
    ...
    return (
      ...
      l.getBlockZ() +
      ":" +
      Float.floatToIntBits(l.getYaw()) +
      ":" +
      Float.floatToIntBits(l.getPitch())
    );
  }
    ...
    for (String s : list) {
      ...
      if (
        s
          ...
          .startsWith(start.toLowerCase(java.util.Locale.ENGLISH))
      ) {
        returned.add(s);
      }
    }
   ...
   */
  @NonNull
  public static CompletableFuture<Chunk> getChunkAtAsync(
   ...
   */
  @NonNull
  public static BlockStateSnapshotResult getBlockState(
    @NonNull Block block,
    boolean useSnapshot
  ) {
      ...
      command -> {
        ...
        if (command.startsWith("[SUDO]")) {
          ...
          if (!Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command)) {
            plugin.logError(
              "Could not execute " + commandType + " command as console: " + command
            );
          }
        }
      }
  ...
  public static NMSAbstraction getNMS()
    ...
    try {
      ...
      plugin.logWarning(
        "No NMS Handler found for " + version + ", falling back to Bukkit API."
      );
    ...
    if (NMSAbstraction.class.isAssignableFrom(clazz)) {
      ...
      throw new IllegalStateException(
        "Class " + clazz.getName() + " does not implement NMSAbstraction"
      );
    }
  }
}
