...
public class BentoBoxVersionCommand extends CompositeCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    user.sendMessage(
      "commands.bentobox.version.plugin-version",
      TextVariables.VERSION,
      getPlugin().getDescription().getVersion()
    );
    user.sendMessage(
      "commands.bentobox.version.database",
      "[database]",
    ...
    getIWM()
      ...
      .forEach(
        e -> {
          ...
          Optional<GameModeAddon> addonOptional = getIWM()
            .getAddon(Bukkit.getWorld(e.getKey()));
          if (addonOptional.isPresent()) {
            ...
            if (
              ...
              getIWM().isNetherGenerate(addon.getOverWorld())
            ) {
          ...
          user.sendMessage(
            user.getTranslation(
              "commands.bentobox.version.game-world",
              TextVariables.NAME,
              e.getKey(),
              "[addon]",
              e.getValue(),
    ...
    getPlugin()
      ...
      .sorted(
        Comparator.comparing(
          o -> o.getDescription().getName().toLowerCase(Locale.ENGLISH)
        )
      )
      .forEach(
        a ->
          user.sendMessage(
            "commands.bentobox.version.addon-syntax",
            TextVariables.NAME,
