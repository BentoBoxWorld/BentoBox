...
public class BStats {
  ...
  private void registerDefaultLanguageChart() {
    metrics.addCustomChart(
      new Metrics.SimplePie(
        "default_language",
        () -> plugin.getSettings().getDefaultLanguage()
      )
    );
  }
  ...
  private void registerDatabaseTypeChart() {
    metrics.addCustomChart(
      new Metrics.SimplePie(
        "database_type",
        () -> plugin.getSettings().getDatabaseType().toString()
      )
    );
  }
  ...
  private void registerIslandsCountChart() {
    metrics.addCustomChart(
      new Metrics.SingleLineChart("islands", () -> plugin.getIslands().getIslandCount())
    );
  }
      ...
      new Metrics.AdvancedPie(
        ...
        () -> {
          ...
          plugin
            ...
            .filter(
              addon ->
                !(addon instanceof GameModeAddon) && addon.getDescription().isMetrics()
            )
      ...
      new Metrics.AdvancedPie(
        ...
        () -> {
          ...
          plugin
            ...
            .forEach(
              gameModeAddon -> values.put(gameModeAddon.getDescription().getName(), 1)
            );
          return values;
        }
      )
      ...
      new Metrics.AdvancedPie(
        ...
        () -> {
          ...
          plugin
            .getHooks()
            .getHooks()
            .forEach(hook -> values.put(hook.getPluginName(), 1));
          return values;
        }
      )
    );
    ...
    metrics.addCustomChart(
      new Metrics.SimplePie(
        ...
        () -> {
          ...
          if (players <= 0) return "0"; else if (players <= 10) return "1-10"; else if (
            players <= 30
          ) return "11-30"; else if (players <= 50) return "31-50"; else if (
            players <= 100
          ) return "51-100"; else if (players <= 150) return "101-150"; else if (
            players <= 200
          ) return "151-200"; else return "201+";
        }
      )
    );
  }
      ...
      new Metrics.AdvancedPie(
        ...
        () -> {
          ...
          Bukkit
            ...
            .forEach(
              player -> {
                Flag.Mode mode = plugin
                  .getPlayers()
                  .getFlagsDisplayMode(player.getUniqueId());
