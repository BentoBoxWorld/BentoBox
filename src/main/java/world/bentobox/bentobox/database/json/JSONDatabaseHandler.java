...
public class JSONDatabaseHandler<T> extends AbstractJSONDatabaseHandler<T> {
  ...
  public List<T> loadObjects() {
    ...
    for (File file : Objects.requireNonNull(
      tableFolder.listFiles(
        (dir, name) -> name.toLowerCase(Locale.ENGLISH).endsWith(JSON)
      )
    )) {
      try (FileReader reader = new FileReader(file)) {
        ...
        plugin.logError(
          "Could not load objects " + file.getName() + " " + e.getMessage()
        );
      }
    }
    return list;
  ...
  public CompletableFuture<Boolean> saveObject(T instance)
    ...
    String path = DATABASE_FOLDER_NAME + File.separator + dataObject.getSimpleName();
    String backupPath =
      DATABASE_FOLDER_NAME + "_backup" + File.separator + dataObject.getSimpleName();
  ...
  private void delete(String uniqueId) {
    ...
    if (tableFolder.exists()) {
      ...
      try {
        ...
        plugin.logError(
          "Could not delete JSON database object! " +
          file.getName() +
          " - " +
          e.getMessage()
        );
