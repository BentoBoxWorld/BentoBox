...
public interface DatabaseSetup {
    ...
    /**
     * Gets the type of database being used. Currently supported options are YAML,
     * JSON, MYSQL, MARIADB and MONGODB. Default is JSON.
     * 
     * @return Database type
     */
    static DatabaseSetup getDatabase() {
        BentoBox plugin = BentoBox.getInstance();
        return Arrays.stream(DatabaseType.values()).filter(plugin.getSettings().getDatabaseType()::equals).findFirst()
                .map(t -> t.database).orElseGet(() -> DatabaseType.JSON.database);
    }
    ...
    /**
     * Gets a database handler that will store and retrieve classes of type
     * dataObjectClass
     * 
     * @param <T>             - Class type
