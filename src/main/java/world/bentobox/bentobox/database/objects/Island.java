...
public class Island implements DataObject {
  ...
  public boolean ban(@NonNull UUID issuer, @NonNull UUID target) {
    ...
    log(
      new LogEntry.Builder("BAN")
        .data("player", target.toString())
        .data("issuer", issuer.toString())
        .build()
    );
    return true;
  }
  ...
  public @NonNull ImmutableSet<UUID> getMemberSet(int minimumRank) {
    ...
    members
      .entrySet()
      .stream()
      .filter(e -> e.getValue() >= minimumRank)
  ...
  public boolean inIslandSpace(Location location) {
    return (
      Util.sameWorld(world, location.getWorld()) &&
      inIslandSpace(location.getBlockX(), location.getBlockZ())
    );
  }
  ...
  public List<Player> getVisitors() {
    return Bukkit
      ...
      .filter(
        player ->
          onIsland(player.getLocation()) &&
          getRank(User.getInstance(player)) == RanksManager.VISITOR_RANK
      )
  ...
  public boolean isBanned(UUID targetUUID) {
    return (
      members.containsKey(targetUUID) &&
      members.get(targetUUID).equals(RanksManager.BANNED_RANK)
    );
  }
  ...
  public boolean onIsland(@NonNull Location target) {
    return (
      Util.sameWorld(world, target.getWorld()) &&
      target.getBlockX() >= getMinProtectedX() &&
      target.getBlockX() < (getMinProtectedX() + protectionRange * 2) &&
      target.getBlockZ() >= getMinProtectedZ() &&
      target.getBlockZ() < (getMinProtectedZ() + protectionRange * 2)
    );
  }
  ...
  public BoundingBox getProtectionBoundingBox() {
    return new BoundingBox(
      ...
      0.0D,
      getMinProtectedZ(),
      getMaxProtectedX() - 1.0D,
      world.getMaxHeight(),
      getMaxProtectedZ() - 1.0D
    );
  }
      ...
      .forEach(
        f ->
          result.put(
            ...
            plugin
              .getIWM()
              .getDefaultIslandFlags(world)
              .getOrDefault(f, f.getDefaultRank())
          )
      );
    plugin
      .getFlagsManager()
      ...
      .forEach(
        f ->
          result.put(
            ...
            plugin
              .getIWM()
              .getDefaultIslandSettings(world)
              .getOrDefault(f, f.getDefaultRank())
          )
      );
  ...
  public boolean showInfo(User user) {
    ...
    if (owner == null) {
      ...
      user.sendMessage(
        "commands.admin.info.owner",
        "[owner]",
        plugin.getPlayers().getName(owner),
        "[uuid]",
        owner.toString()
      );
      ...
      long lastPlayed = (Bukkit.getServer().getOfflinePlayer(owner).getLastPlayed() != 0)
        ? Bukkit.getServer().getOfflinePlayer(owner).getLastPlayed()
        : Bukkit.getServer().getOfflinePlayer(owner).getFirstPlayed();
      user.sendMessage(
        "commands.admin.info.last-login",
        "[date]",
        new Date(lastPlayed).toString()
      );

      user.sendMessage(
        "commands.admin.info.deaths",
        "[number]",
      ...
      String total = plugin.getIWM().getResetLimit(world) < 0
        ? "Unlimited"
        : String.valueOf(plugin.getIWM().getResetLimit(world));
      user.sendMessage(
        "commands.admin.info.resets-left",
        "[number]",
        resets,
        "[total]",
        total
    ...
    Vector to = center
      .toVector()
      .add(new Vector(range - 1, 0, range - 1))
      .setY(center.getWorld().getMaxHeight());
    user.sendMessage(
      "commands.admin.info.island-coords",
      "[xz1]",
      Util.xyz(from),
      "[xz2]",
      Util.xyz(to)
    ...
    user.sendMessage(
      "commands.admin.info.protection-range",
      "[range]",
      String.valueOf(protectionRange)
    );
    user.sendMessage(
      "commands.admin.info.max-protection-range",
      "[range]",
      String.valueOf(maxEverProtectionRange)
    );
    Vector pfrom = center
      .toVector()
      .subtract(new Vector(protectionRange, 0, protectionRange))
      .setY(0);
    Vector pto = center
      .toVector()
      .add(new Vector(protectionRange - 1, 0, protectionRange - 1))
      .setY(center.getWorld().getMaxHeight());
    user.sendMessage(
      "commands.admin.info.protection-coords",
      "[xz1]",
      Util.xyz(pfrom),
      "[xz2]",
      Util.xyz(pto)
    ...
    if (!banned.isEmpty()) {
      ...
      banned.forEach(
        u ->
          user.sendMessage(
            "commands.admin.info.banned-format",
            TextVariables.NAME,
  ...
  public void showMembers(User user) {
    ...
    members.forEach(
      (u, i) -> {
        if (owner.equals(u)) {
          user.sendMessage(
            "commands.admin.info.team-owner-format",
            TextVariables.NAME,
            plugin.getPlayers().getName(u),
            "[rank]",
          ...
          user.sendMessage(
            "commands.admin.info.team-member-format",
            TextVariables.NAME,
            plugin.getPlayers().getName(u),
            "[rank]",
            user.getTranslation(plugin.getRanksManager().getRank(i))
  ...
  public void toggleFlag(Flag flag) {
    if (
      flag.getType().equals(Flag.Type.SETTING) ||
      flag.getType().equals(Flag.Type.WORLD_SETTING)
    ) {
  ...
  public boolean hasNetherIsland() {
    ...
    return (
      iwm.isNetherGenerate(getWorld()) &&
      iwm.isNetherIslands(getWorld()) &&
      iwm.getNetherWorld(getWorld()) != null &&
      !getCenter()
        .toVector()
        .toLocation(iwm.getNetherWorld(getWorld()))
        .getBlock()
        .getType()
        .equals(Material.AIR)
    );
  ...
  public boolean hasEndIsland() {
    IslandWorldManager iwm = BentoBox.getInstance().getIWM();
    return (
      iwm.isEndGenerate(getWorld()) &&
      iwm.isEndIslands(getWorld()) &&
      iwm.getEndWorld(getWorld()) != null &&
      !getCenter()
        .toVector()
        .toLocation(iwm.getEndWorld(getWorld()))
        .getBlock()
        .getType()
        .equals(Material.AIR)
    );
  ...
  public int getRankCommand(String command) {
    return commandRanks == null
      ? RanksManager.OWNER_RANK
      : commandRanks.getOrDefault(command, RanksManager.OWNER_RANK);
  }
