...
public class SQLiteDatabaseHandler<T> extends SQLDatabaseHandler<T> {
   ...
   */
  protected SQLiteDatabaseHandler(
    BentoBox plugin,
    Class<T> type,
  ...
  protected void createSchema() {
    ...
    try (
      PreparedStatement pstmt = getConnection()
        .prepareStatement(getSqlConfig().getSchemaSQL())
    ) {
      pstmt.execute();
    } catch (SQLException e) {
      plugin.logError(
  ...
  private void rename(PreparedStatement pstmt) {
    try (ResultSet resultSet = pstmt.executeQuery()) {
      if (resultSet.next() && resultSet.getBoolean(1)) {
        ...
        String sql = getSqlConfig()
          .getRenameTableSQL()
          .replace(
            "[oldTableName]",
            getSqlConfig()
              .getOldTableName()
              .replace("[tableName]", getSqlConfig().getTableName())
          );
      ...
      () -> {
        try (
          PreparedStatement preparedStatement = getConnection()
            .prepareStatement(getSqlConfig().getSaveObjectSQL())
        ) {
