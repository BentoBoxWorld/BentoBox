...
public class SQLiteDatabaseHandler<T> extends SQLDatabaseHandler<T> {
    ...
    protected SQLiteDatabaseHandler(BentoBox plugin, Class<T> type, DatabaseConnector databaseConnector) {
        super(plugin, type, databaseConnector, new SQLConfiguration(plugin, type).schema(
                "CREATE TABLE IF NOT EXISTS `[tableName]` (json JSON, uniqueId VARCHAR(255) NOT NULL PRIMARY KEY)")
                .saveObject(
                        "INSERT INTO `[tableName]` (json, uniqueId) VALUES (?, ?) ON CONFLICT(uniqueId) DO UPDATE SET json = ?")
    ...
    protected void createSchema() {
        if (this.getSqlConfig().renameRequired()) {
            // SQLite does not have a rename if exists command so we have to manually check
            // if the old table exists
            ...
            try (Connection connection = this.dataSource.getConnection();
                ...
                this.plugin.logError("Could not check if " + this.getSqlConfig().getOldTableName()
                        + " exists for data object " + this.dataObject.getCanonicalName() + " " + e.getMessage());
            }
        }
    ...
    private void rename(PreparedStatement pstmt) {
        try (ResultSet resultSet = pstmt.executeQuery()) {
            if (resultSet.next() && resultSet.getBoolean(1)) {
                ...
                String sql = this.getSqlConfig().getRenameTableSQL().replace("[oldTableName]", this.getSqlConfig()
                        .getOldTableName().replace("[tableName]", this.getSqlConfig().getTableName()));
        ...
        this.processQueue.add(() -> {
            try (Connection connection = this.dataSource.getConnection();
                    PreparedStatement preparedStatement = connection
                            .prepareStatement(this.getSqlConfig().getSaveObjectSQL())) {
                ...
                this.plugin.logError("Could not save object " + instance.getClass().getName() + " "
                        + ((DataObject) instance).getUniqueId() + " " + e.getMessage());
                completableFuture.complete(false);
            }
        });
