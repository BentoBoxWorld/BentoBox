...
/**
 * Contains the SQL strings for the database. The default strings are for MySQL,
 * so only the deltas need to be supplied.
 * 
 * @author tastybento
 *
 */
public class SQLConfiguration {
    ...
    public <T> SQLConfiguration(BentoBox plugin, Class<T> type) {
        ...
        this.schema(
                "CREATE TABLE IF NOT EXISTS `[tableName]` (json JSON, uniqueId VARCHAR(255) GENERATED ALWAYS AS (json->\"$.uniqueId\"), UNIQUE INDEX i (uniqueId) )");
        ...
        this.renameTable("SELECT Count(*) INTO @exists " + "FROM information_schema.tables " + "WHERE table_schema = '"
                + plugin.getSettings().getDatabaseName() + "' " + "AND table_type = 'BASE TABLE' "
