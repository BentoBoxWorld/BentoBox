...
public class SQLConfiguration {
  ...
  public <T>SQLConfiguration(BentoBox plugin, Class<T> type) {
    ...
    saveObject(
      "INSERT INTO `[tableName]` (json) VALUES (?) ON DUPLICATE KEY UPDATE json = ?"
    );
    deleteObject("DELETE FROM `[tableName]` WHERE uniqueId = ?");
    objectExists(
      "SELECT IF ( EXISTS( SELECT * FROM `[tableName]` WHERE `uniqueId` = ?), 1, 0)"
    );
    renameTable(
  ...
  public SQLConfiguration renameTable(String string) {
    this.renameTableSQL =
      string.replace(TABLE_NAME, tableName).replace("\\[oldTableName\\]", oldTableName);
    return this;
  }
