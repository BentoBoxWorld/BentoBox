...
public class YamlDatabaseHandler<T> extends AbstractDatabaseHandler<T> {
   ...
   */
  @Override
  public T loadObject(String key)
    throws InstantiationException, IllegalAccessException, InvocationTargetException, ClassNotFoundException, IntrospectionException, NoSuchMethodException {
    ...
    YamlConfiguration config =
      ((YamlDatabaseConnector) databaseConnector).loadYamlFile(path, key);
  ...
  public List<T> loadObjects()
    ...
    FilenameFilter ymlFilter = (dir, name) ->
      name.toLowerCase(java.util.Locale.ENGLISH).endsWith(YML);
    ...
    for (File file : Objects.requireNonNull(tableFolder.listFiles(ymlFilter))) {
      ...
      YamlConfiguration config =
        ((YamlDatabaseConnector) databaseConnector).loadYamlFile(
            DATABASE_FOLDER_NAME + File.separator + dataObject.getSimpleName(),
            fileName
          );
   ...
   */
  private T createObject(YamlConfiguration config)
    throws InstantiationException, IllegalAccessException, IntrospectionException, InvocationTargetException, ClassNotFoundException, NoSuchMethodException {
    ...
    for (Field field : dataObject.getDeclaredFields()) {
      ...
      String storageLocation = (configEntry != null && !configEntry.path().isEmpty())
        ? configEntry.path()
        : field.getName();
      ...
      if (
        adapterNotation != null &&
        AdapterInterface.class.isAssignableFrom(adapterNotation.value())
      ) {
        ...
        method.invoke(
          instance,
          (
            (AdapterInterface<?, ?>) adapterNotation
              .value()
  ...
  private void deserializeValue(
    ...
    if (value != null && !value.getClass().equals(MemorySection.class)) {
      ...
      if (
        !(
          Enum.class.isAssignableFrom(propertyDescriptor.getPropertyType()) &&
          setTo == null
        )
      ) {
        ...
        try {
          ...
          Type setType = propertyDescriptor
            .getWriteMethod()
          ...
          plugin.logError(
            "Could not deserialize. Attempt by " +
        ...
        plugin.logError(
          "Default setting value will be used: " +
  ...
  private void deserializeMap(
    ...
    if (config.getConfigurationSection(storageLocation) != null) {
      ...
      for (String key : config.getConfigurationSection(storageLocation).getKeys(false)) {
        ...
        Object mapValue = deserialize(
          config.get(storageLocation + "." + key),
  ...
  public CompletableFuture<Boolean> saveObject(T instance)
    ...
    for (Field field : dataObject.getDeclaredFields()) {
      ...
      if (
        Map.class.isAssignableFrom(propertyDescriptor.getPropertyType()) && value != null
      ) {
        serializeMap((Map<Object, Object>) value, config, storageLocation);
      } else if (
        Set.class.isAssignableFrom(propertyDescriptor.getPropertyType()) && value != null
      ) {
      ...
      processQueue.add(
        () ->
          completableFuture.complete(
            ((YamlDatabaseConnector) databaseConnector).saveYamlFile(
                data,
                path,
                name,
                yamlComments
  ...
  private void handleComments(
    ...
    @NonNull Map<String, String> yamlComments,
    @NonNull String parent
  ) {
    // See if there are multiple comments
    ConfigComment.Line comments = annotatedElement.getAnnotation(
      ConfigComment.Line.class
    );
    if (comments != null) {
   ...
   * @since 1.3.0
   */
  private void handleConfigEntryComments(
    @NonNull ConfigEntry configEntry,
    @NonNull YamlConfiguration config,
    @NonNull Map<String, String> yamlComments,
    @NonNull String parent
  ) {
    ...
    if (!configEntry.video().isEmpty()) {
      setComment(
        "You can find more details in this video: " + configEntry.video(),
        config,
        yamlComments,
        parent
      );
    ...
    if (Enum.class.isAssignableFrom(clazz)) {
      ...
      try {
        ...
        if (name.equals("PIG_ZOMBIE") || name.equals("ZOMBIFIED_PIGLIN")) {
          return Enums
            .getIfPresent(EntityType.class, "ZOMBIFIED_PIGLIN")
  ...
  private void delete(String uniqueId) {
    ...
    if (tableFolder.exists()) {
      ...
      try {
        ...
        plugin.logError(
          "Could not delete yml database object! " +
          file.getName() +
          " - " +
          e.getMessage()
        );
