...
public class Database<T> {
  ...
  public List<T> loadObjects() {
    ...
    try {
      ...
      logger.severe(
        () -> "Could not load objects from database! Error: " + e.getMessage()
      );
    }
    return result;
  }
  ...
  public boolean saveObject(T instance) {
    saveObjectAsync(instance)
      .thenAccept(
        r -> {
          if (Boolean.FALSE.equals(r)) logger.severe(
            () -> "Could not save object to database!"
          );
        }
      );
    return true;
