...
public abstract class AbstractDatabaseHandler<T> {
    ...
    /**
     * FIFO queue for saves or deletions. Note that the assumption here is that most
     * database objects will be held in memory because loading is not handled with
     * this queue. That means that it is theoretically possible to load something
     * before it has been saved. So, in general, load your objects and then save
     * them async only when you do not need the data again immediately.
     */
     ...
     *                          database like host/port/database/user/password
     */
    protected AbstractDatabaseHandler(BentoBox plugin, Class<T> type, DatabaseConnector databaseConnector) {
        ...
        asyncSaveTask = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, () -> {
            ...
            if (shutdown || plugin.isShutdown()) {
                // Cancel - this will only get called if the plugin is shutdown separately to
                // the server
     ...
     * @return list of <T>
     */
    public abstract List<T> loadObjects() throws InstantiationException, IllegalAccessException,
     ...
     */
    public abstract CompletableFuture<Boolean> saveObject(T instance)
            throws IllegalAccessException, InvocationTargetException, IntrospectionException;

    /**
     * Deletes the object with the unique id from the database. If the object does
     * not exist, it will fail silently. Use {@link #objectExists(String)} if you
     * need to know if the object is in the database beforehand.
     * 
     * @param instance - object instance
     */
    public abstract void deleteObject(T instance)
            throws IllegalAccessException, InvocationTargetException, IntrospectionException;
     ...
     * exist, it will fail silently. Use {@link #objectExists(String)} if you need
     * to know if the object is in the database beforehand.
     * 
