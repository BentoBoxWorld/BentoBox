...
public class WebManager {
  ...
  @NonNull
  private List<CatalogEntry> addonsCatalog;

  @NonNull
  private List<CatalogEntry> gamemodesCatalog;
  ...
  public WebManager(@NonNull BentoBox plugin) {
    ...
    if (plugin.getSettings().isGithubDownloadData()) {
      ...
      long connectionInterval =
        plugin.getSettings().getGithubConnectionInterval() * 20L * 60L;
      if (connectionInterval <= 0) {
        // If below 0, it means we shouldn't run this as a repeating task.
        plugin
          .getServer()
          .getScheduler()
          .runTaskLaterAsynchronously(plugin, this::requestGitHubData, 20L);
      } else {
      ...
      .ifPresent(
        gh -> {
          ...
          try {
            ...
            if (plugin.getSettings().isLogGithubDownloadData()) {
              plugin.logError(
                "An unhandled exception occurred when connecting to the GitHub weblink.."
              );
          ...
          repositories.addAll(
            plugin
              ...
              .getEnabledAddons()
              .stream()
              .map(addon -> addon.getDescription().getRepository())
          ...
          if (plugin.getSettings().isLogGithubDownloadData()) {
            plugin.log(
              "Gathering contribution data for: " + String.join(", ", repositories)
            );
          }
          ...
          for (String repository : repositories) {
            ...
            try {
              ...
              if (plugin.getSettings().isLogGithubDownloadData()) {
                plugin.logError(
                  "An unhandled exception occurred when gathering contributors data from the '" +
                  repository +
                  "' repository..."
                );
                plugin.logStacktrace(e);
  ...
  private void parseCatalogContent(
    String tagsContent,
    ...
    if (!tagsContent.isEmpty()) {
      try {
          ...
          .forEach(
            entry ->
              plugin
                ...
                .forEach(
                  locale -> {
                    JsonElement translation = entry
                      .getValue()
                      .getAsJsonObject()
                      .get(locale.toLanguageTag());
                    if (translation != null) {
                      locale.set(
                        "catalog.tags." + entry.getKey(),
                        translation.getAsString()
                      );
                    }
                  }
                )
        ...
        if (plugin.getSettings().isLogGithubDownloadData()) {
          plugin.log(
            "Could not update the Catalog Tags: the gathered JSON data is malformed."
          );
        }
      }
    }
    ...
    if (!topicsContent.isEmpty()) {
      try {
        ...
        if (plugin.getSettings().isLogGithubDownloadData()) {
          plugin.log(
            "Could not update the Catalog Topics: the gathered JSON data is malformed."
          );
        }
      }
    }
    ...
    if (!catalogContent.isEmpty()) {
      try {
        ...
        catalog
          ...
          .forEach(
            gamemode -> gamemodesCatalog.add(new CatalogEntry(gamemode.getAsJsonObject()))
          );
        catalog
          .getAsJsonArray("addons")
          .forEach(addon -> addonsCatalog.add(new CatalogEntry(addon.getAsJsonObject())));
      } catch (JsonParseException e) {
        if (plugin.getSettings().isLogGithubDownloadData()) {
          plugin.log(
            "Could not update the Catalog content: the gathered JSON data is malformed."
          );
        }
      }
    }
  ...
  private String getContent(@NonNull GitHubRepository repo, String fileName) {
    try {
      ...
      if (plugin.getSettings().isLogGithubDownloadData()) {
        plugin.logError(
          "An unhandled exception occurred when downloading '" +
          fileName +
          "' from GitHub..."
        );
        plugin.logStacktrace(e);
    ...
    try {
      ...
      for (GitHubContributor gitHubContributor : repo.getContributors()) {
        addonContributors.add(
          new Contributor(
            gitHubContributor.getUsername(),
            gitHubContributor.getContributionsAmount()
          )
        );
      }
