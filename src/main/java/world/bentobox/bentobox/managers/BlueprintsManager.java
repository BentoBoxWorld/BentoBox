...
public class BlueprintsManager {
   ...
   */
  @NonNull
  private Map<GameModeAddon, List<BlueprintBundle>> blueprintBundles;
   ...
   */
  @NonNull
  private Map<GameModeAddon, List<Blueprint>> blueprints;
  ...
  @NonNull
  private Set<GameModeAddon> blueprintsLoaded;
  ...
  public void extractDefaultBlueprints(@NonNull GameModeAddon addon) {
    ...
    if (!folder.exists() && !folder.mkdirs()) {
      ...
      plugin.logError(
        "This might be due to incorrectly set-up write permissions on the operating system."
      );
      return;
    }
    ...
    try (JarFile jar = new JarFile(addon.getFile())) {
      Util
        .listJarFiles(jar, FOLDER_NAME, BLUEPRINT_BUNDLE_SUFFIX)
        .forEach(name -> addon.saveResource(name, false));
      Util
        .listJarFiles(jar, FOLDER_NAME, BLUEPRINT_SUFFIX)
        .forEach(name -> addon.saveResource(name, false));
    } catch (IOException e) {
  ...
  public Map<String, BlueprintBundle> getBlueprintBundles(@NonNull GameModeAddon addon) {
    ...
    return blueprintBundles
      .get(addon)
      .stream()
      .collect(Collectors.toMap(BlueprintBundle::getUniqueId, b -> b));
  }
  ...
  public BlueprintBundle getDefaultBlueprintBundle(@NonNull GameModeAddon addon) {
    if (blueprintBundles.containsKey(addon)) {
      return blueprintBundles
        .get(addon)
        .stream()
        .filter(bb -> bb.getUniqueId().equals(DEFAULT_BUNDLE_NAME))
        .findFirst()
        .orElse(null);
    }
    return null;
  ...
  private boolean loadBundles(@NonNull GameModeAddon addon) {
    ...
    if (!bpf.exists()) {
      plugin.logError(
        "There is no blueprint folder for addon " + addon.getDescription().getName()
      );
      bpf.mkdirs();
    }
    boolean loaded = false;
    File[] bundles = bpf.listFiles(
      (dir, name) -> name.toLowerCase(Locale.ENGLISH).endsWith(BLUEPRINT_BUNDLE_SUFFIX)
    );
      ...
      try {
        ...
        if (bb != null) {
          ...
          if (
            ...
            plugin.logWarning(
              "Could not load blueprint bundle '" +
              file.getName() +
              FOR +
              addon.getDescription().getName() +
              "."
            ...
            plugin.logWarning(
              "The uniqueId '" +
              bb.getUniqueId() +
              "' is already used by another Blueprint Bundle."
            );
            plugin.logWarning(
              "This can occur if the Blueprint Bundles' files were manually edited."
            );
            plugin.logWarning(
  ...
  private BlueprintBundle getDefaultBlueprintBundle() {
    ...
    bb.setDescription(
      Collections.singletonList(ChatColor.AQUA + "Default bundle of blueprints")
    );
    return bb;
  }
  ...
  private Blueprint getDefaultBlueprint() {
    ...
    defaultBp.setDescription(
      Collections.singletonList(ChatColor.AQUA + "A bedrock block")
    );
  ...
  private void makeDefaults(@NonNull GameModeAddon addon) {
    ...
    new BlueprintClipboardManager(plugin, getBlueprintsFolder(addon))
    .saveBlueprint(defaultBp);
  ...
  public void loadBlueprints(@NonNull GameModeAddon addon) {
    ...
    File[] bps = bpf.listFiles(
      (dir, name) -> name.toLowerCase(Locale.ENGLISH).endsWith(BLUEPRINT_SUFFIX)
    );
    ...
    for (File file : bps) {
      String fileName = file
        .getName()
        .substring(0, file.getName().length() - BLUEPRINT_SUFFIX.length());
      try {
        Blueprint bp = new BlueprintClipboardManager(plugin, bpf).loadBlueprint(fileName);
        bp.setName(fileName);
        blueprints.get(addon).add(bp);
        plugin.log(
          "Loaded blueprint '" + bp.getName() + FOR + addon.getDescription().getName()
        );
      } catch (Exception e) {
  ...
  public void addBlueprint(@NonNull GameModeAddon addon, @NonNull Blueprint bp) {
    ...
    blueprints.get(addon).add(bp);
    plugin.log(
      "Added blueprint '" + bp.getName() + FOR + addon.getDescription().getName()
    );
  }
  ...
  public boolean saveBlueprint(@NonNull GameModeAddon addon, @NonNull Blueprint bp) {
    return new BlueprintClipboardManager(plugin, getBlueprintsFolder(addon))
    .saveBlueprint(bp);
  }
      ...
      .runTaskAsynchronously(
        ...
        () -> {
          ...
          File fileName = new File(
            bpf,
            sanitizeFileName(bb.getUniqueId()) + BLUEPRINT_BUNDLE_SUFFIX
          );
  ...
  public Map<String, Blueprint> getBlueprints(GameModeAddon addon) {
    ...
    return blueprints
      .get(addon)
      .stream()
      .collect(Collectors.toMap(Blueprint::getName, b -> b));
  }
  ...
  public boolean paste(GameModeAddon addon, Island island, String name, Runnable task) {
    ...
    // Paste
    if (bp != null) {
      new BlueprintPaster(plugin, bp, addon.getOverWorld(), island)
        .paste()
        .thenAccept(
          b ->
            pasteNether(addon, bb, island)
              .thenAccept(
                b2 ->
                  pasteEnd(addon, bb, island)
  ...
  private CompletableFuture<Boolean> pasteNether(
    GameModeAddon addon,
    BlueprintBundle bb,
    Island island
  ) {
    if (
      bb.getBlueprint(World.Environment.NETHER) != null &&
  ...
  private CompletableFuture<Boolean> pasteEnd(
    GameModeAddon addon,
    BlueprintBundle bb,
    Island island
  ) {
    // Make end island
    if (
      bb.getBlueprint(World.Environment.THE_END) != null &&
  ...
  public @Nullable String validate(GameModeAddon addon, String name) {
    ...
    if (
      blueprintBundles.containsKey(addon) &&
      getBlueprintBundles(addon).containsKey(name.toLowerCase(Locale.ENGLISH))
    ) {
      return name;
    }
    return null;
  ...
  public boolean checkPerm(
    ...
    BlueprintBundle bb = getBlueprintBundles((GameModeAddon) addon)
      .get(name.toLowerCase(Locale.ENGLISH));
    if (
      bb == null ||
      (
        bb.isRequirePermission() &&
        !name.equals(DEFAULT_BUNDLE_NAME) &&
        !user.hasPermission(permission)
      )
    ) {
      user.sendMessage(
        "general.errors.no-permission",
        TextVariables.PERMISSION,
        permission
      );
      return false;
    }
