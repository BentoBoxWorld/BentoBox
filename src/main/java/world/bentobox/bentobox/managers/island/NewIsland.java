  ...
  public void newIsland(Island oldIsland) throws IOException {
    ...
    if (next == null) {
      ...
      if (next == null) {
        ...
        plugin.logError(
          "If the world was imported, try multiple times until all unowned islands are known."
        );
    ...
    Runnable task = () -> {
      ...
      if (island.getSpawnPoint(Environment.NORMAL) != null) {
        plugin
          .getPlayers()
          .setHomeLocation(user, island.getSpawnPoint(Environment.NORMAL), 1);
      }
      // Stop the player from falling or moving if they are
      if (user.isOnline()) {
        if (
          reason.equals(Reason.RESET) ||
          (
            reason.equals(Reason.CREATE) &&
          ...
          plugin
            .getIslands()
            .homeTeleportAsync(world, user.getPlayer(), true)
            .thenRun(() -> tidyUp(oldIsland));
          return;
        } else {
