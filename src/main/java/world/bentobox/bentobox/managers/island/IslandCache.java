...
public class IslandCache {
  ...
  public boolean addIsland(@NonNull Island island) {
    ...
    if (addToGrid(island)) {
      ...
      if (island.isOwned()) {
        ...
        island
          .getMemberSet()
          .forEach(member -> islandsByUUID.get(island.getWorld()).put(member, island));
      }
      return true;
    }
    return false;
  ...
  public void addPlayer(@NonNull UUID uuid, @NonNull Island island) {
    islandsByUUID
      .computeIfAbsent(island.getWorld(), k -> new HashMap<>())
      .put(uuid, island);
  }
  ...
  private boolean addToGrid(@NonNull Island newIsland) {
    return grids
      .computeIfAbsent(newIsland.getWorld(), k -> new IslandGrid())
      .addToGrid(newIsland);
  }
  ...
  public boolean deleteIslandFromCache(@NonNull Island island) {
    if (
      !islandsByLocation.remove(island.getCenter(), island) ||
      !islandsByUUID.containsKey(island.getWorld())
    ) {
      return false;
    }
    ...
    islandsByUUID
      .get(island.getWorld())
      .entrySet()
      .removeIf(en -> en.getValue().equals(island));
    // Remove from grid
  ...
  public void deleteIslandFromCache(@NonNull String uniqueId) {
    ...
    islandsByUUID
      .values()
      .forEach(m -> m.values().removeIf(i -> i.getUniqueId().equals(uniqueId)));
  }
  ...
  public Island get(@NonNull World world, @NonNull UUID uuid) {
    return islandsByUUID.containsKey(Util.getWorld(world))
      ? islandsByUUID.get(Util.getWorld(world)).get(uuid)
      : null;
  }
  ...
  public Island getIslandAt(@NonNull Location location) {
    ...
    return grids
      .get(Util.getWorld(location.getWorld()))
      .getIslandAt(location.getBlockX(), location.getBlockZ());
  }
  ...
  public Collection<Island> getIslands(@NonNull World world) {
    ...
    List<Island> islandsInWorld = islandsByLocation
      .entrySet()
      .stream()
  ...
  public Set<UUID> getMembers(@NonNull World world, @NonNull UUID uuid, int minimumRank) {
    Island island = islandsByUUID
      .computeIfAbsent(Util.getWorld(world), k -> new HashMap<>())
      .get(uuid);
  ...
  public void setOwner(@NonNull Island island, @Nullable UUID newOwnerUUID) {
    ...
    if (newOwnerUUID != null) {
      islandsByUUID
        .computeIfAbsent(Util.getWorld(island.getWorld()), k -> new HashMap<>())
        .put(newOwnerUUID, island);
    }
    islandsByLocation.put(island.getCenter(), island);
    islandsById.put(island.getUniqueId(), island);
  }
  ...
  public void removeIsland(@NonNull Island island) {
    ...
    if (islandsByUUID.containsKey(Util.getWorld(island.getWorld()))) {
      islandsByUUID
        .get(Util.getWorld(island.getWorld()))
        .values()
        .removeIf(island::equals);
    }
  ...
  public void resetAllFlags(World world) {
    ...
    islandsById
      .values()
      .stream()
      .filter(i -> i.getWorld().equals(w))
      .forEach(Island::setFlagsDefaults);
  }
  ...
  public void resetFlag(World world, Flag flag) {
    World w = Util.getWorld(world);
    int setting = BentoBox
      .getInstance()
      .getIWM()
      .getDefaultIslandFlags(w)
      .getOrDefault(flag, flag.getDefaultRank());
    islandsById
      .values()
      .stream()
      .filter(i -> i.getWorld().equals(w))
      .forEach(i -> i.setFlag(flag, setting));
  }
