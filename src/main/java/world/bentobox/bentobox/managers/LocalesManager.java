...
public class LocalesManager {
  ...
  public String get(String reference) {
    ...
    if (
      languages.containsKey(
        Locale.forLanguageTag(plugin.getSettings().getDefaultLanguage())
      ) &&
      languages
        .get(Locale.forLanguageTag(plugin.getSettings().getDefaultLanguage()))
        .contains(reference)
    ) {
      return languages
        .get(Locale.forLanguageTag(plugin.getSettings().getDefaultLanguage()))
  ...
  void copyLocalesFromAddonJar(Addon addon) {
    try (JarFile jar = new JarFile(addon.getFile())) {
      ...
      Util
        .listJarFiles(jar, LOCALE_FOLDER, ".yml")
        .forEach(
          lf -> {
  ...
  private void copyLocalesFromPluginJar() {
    ...
    File localeDir = new File(
      plugin.getDataFolder(),
      LOCALE_FOLDER + File.separator + LocalesManager.BENTOBOX
    );
  ...
  public void loadLocalesFromFile(String localeFolder) {
    ...
    File localeDir = new File(
      plugin.getDataFolder(),
      LOCALE_FOLDER + File.separator + localeFolder
    );
    if (!localeDir.exists()) {
    ...
    for (File language : Objects.requireNonNull(localeDir.listFiles(ymlFilter))) {
      Locale localeObject = Locale.forLanguageTag(
        language.getName().substring(0, language.getName().length() - 4)
      );
  ...
  public List<Locale> getAvailableLocales(boolean sort) {
    if (sort) {
      ...
      locales.sort(
        (locale1, locale2) -> {
          if (
            locale1.toLanguageTag().equals(plugin.getSettings().getDefaultLanguage())
          ) return -2; else if (
            locale1.toLanguageTag().startsWith("en")
          ) return -1; else if (
            locale1.toLanguageTag().equals(locale2.toLanguageTag())
  ...
  public void analyzeLocales(boolean fix) {
    ...
    if (languages.containsKey(Locale.US)) {
      ...
      user.sendRawMessage(
        ChatColor.RED + "No US English in BentoBox to use for analysis!"
      );
    }
    ...
    plugin
      ...
      .forEach(
        addon -> {
          user.sendRawMessage(ChatColor.AQUA + SPACER);
          user.sendRawMessage(
            ChatColor.AQUA + "Analyzing addon " + addon.getDescription().getName()
          );
          user.sendRawMessage(ChatColor.AQUA + SPACER);
          languages.clear();
          loadLocalesFromFile(addon.getDescription().getName());
  ...
  private void analyze(User user) {
    ...
    languages
      .values()
      .stream()
      .filter(l -> !l.toLanguageTag().equals(Locale.US.toLanguageTag()))
      .forEach(
        l -> {
          user.sendRawMessage(ChatColor.GREEN + SPACER);
          user.sendRawMessage(
            ChatColor.GREEN + "Analyzing locale file " + l.toLanguageTag() + ":"
          );
          ...
          if (complete) {
            ...
            user.sendRawMessage(
              ChatColor.RED + "The following YAML is missing. Please translate it:"
            );
