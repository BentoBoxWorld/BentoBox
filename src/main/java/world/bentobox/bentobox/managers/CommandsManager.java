  ...
  public void registerCommand(@NonNull CompositeCommand command) {
    ...
    try {
      ...
      if (command.getAddon() != null) {
        commandPrefix =
          command.getAddon().getDescription().getName().toLowerCase(Locale.ENGLISH);
      }
      if (!commandMap.register(commandPrefix, command)) {
        BentoBox
          .getInstance()
          .logError(
            "Failed to register command " + commandPrefix + " " + command.getLabel()
          );
      }
      ...
      Bukkit
        .getLogger()
        .severe(
          "Bukkit server commandMap method is not there! This means no commands can be registered!"
        );
    }
  }
  ...
  public void unregisterCommands() {
    ...
    try {
      ...
      Bukkit
        .getLogger()
        .severe(
          "Known commands reflection was not possible, BentoBox is now unstable, so restart server!"
        );
    }
  }
