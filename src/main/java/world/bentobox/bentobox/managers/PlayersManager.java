...
public class PlayersManager {
    ...
    if (!schedule) {
      for (Players player : playerCache.values()) {
        try {
          ...
          plugin.logError(
            "Could not save player to database when running sync! " + e.getMessage()
          );
        }
      }
      return;
    ...
    if (!playerCache.containsKey(playerUUID)) {
      ...
      if (handler.objectExists(playerUUID.toString())) {
        ...
        if (player == null) {
          ...
          plugin.logError(
            "Corrupted player database entry for " +
            playerUUID +
            " - unrecoverable. Recreated."
          );
  ...
  public boolean isKnown(UUID uniqueID) {
    return (
      uniqueID != null &&
      (playerCache.containsKey(uniqueID) || handler.objectExists(uniqueID.toString()))
    );
  }
  ...
  public int getResetsLeft(World world, UUID playerUUID) {
    ...
    if (plugin.getIWM().getResetLimit(world) == -1) {
      ...
      return Math.max(
        plugin.getIWM().getResetLimit(world) - getResets(world, playerUUID),
        0
      );
    }
  }
  ...
  public int getDeaths(World world, UUID playerUUID) {
    addPlayer(playerUUID);
    return playerCache.get(playerUUID) == null
      ? 0
      : playerCache.get(playerUUID).getDeaths(world);
  }
  ...
  public void removePlayer(Player player) {
    ...
    playerCache
      .values()
      .removeIf(p -> player.getName().equalsIgnoreCase(p.getPlayerName()));
    // Remove if the player's UUID is the same
    playerCache
      .values()
      .removeIf(p -> player.getUniqueId().toString().equals(p.getUniqueId()));
  }
}
