...
public class VaultHook extends Hook {
  ...
  public double getBalance(User user, World world) {
    if (!user.isOfflinePlayer()) return 0.0D;

    if (world == null) return economy.getBalance(user.getOfflinePlayer());
  ...
  public EconomyResponse withdraw(User user, double amount, World world) {
    ...
    if (world == null) return economy.withdrawPlayer(user.getOfflinePlayer(), amount);
    ...
    if (
      response == null || response.type == ResponseType.NOT_IMPLEMENTED
  ...
  public EconomyResponse deposit(User user, double amount, World world) {
    ...
    if (world == null) return economy.depositPlayer(user.getOfflinePlayer(), amount);
  ...
  public boolean has(User user, double amount, World world) {
    ...
    if (world == null) return economy.has(user.getOfflinePlayer(), amount);
