...
public class DynmapHook extends Hook {
  ...
  public boolean hook() {
    try {
      ...
      BentoBox
        ...
        .getAddonsManager()
        .getGameModeAddons()
        .forEach(this::registerMarkerSet);
  ...
  public void registerMarkerSet(@NonNull GameModeAddon addon) {
    ...
    if (getMarkerSet(addon) == null) {
      ...
      MarkerSet set = markerAPI.createMarkerSet(
        name.toLowerCase(Locale.ENGLISH) + ".markers",
        name,
        null,
        true
      );
  ...
  public MarkerSet getMarkerSet(@NonNull GameModeAddon addon) {
    if (markerSets.containsKey(addon)) {
      ...
      return markerAPI.getMarkerSet(
        addon.getDescription().getName().toLowerCase(Locale.ENGLISH) + ".markers"
      );
    }
  }
