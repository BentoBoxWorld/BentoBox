...
public class Flag implements Comparable<Flag> {
  ...
  private PanelItemBuilder createSettingFlag(
    User user,
    Island island,
    ...
    if (island != null) {
      ...
      if (this.cooldown > 0 && island.isCooldown(this)) {
        pib.description(
          user.getTranslation("protection.panel.flag-item.setting-cooldown")
        );
      }
    }
    return pib;
  ...
  private PanelItemBuilder createProtectionFlag(
    BentoBox plugin,
    User user,
    Island island,
    PanelItemBuilder pib
