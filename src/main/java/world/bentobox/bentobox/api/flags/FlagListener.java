...
public abstract class FlagListener implements Listener {
  ...
  public boolean checkIsland(
    ...
    if (flag.getType().equals(Flag.Type.SETTING)) {
      ...
      if (island.isPresent()) {
        report(
          ...
          loc,
          flag,
          island.map(x -> x.isAllowed(flag)).orElse(false)
            ? Why.SETTING_ALLOWED_ON_ISLAND
        ...
        report(
          ...
          loc,
          flag,
          flag.isSetForWorld(loc.getWorld())
            ? Why.SETTING_ALLOWED_IN_WORLD
            : Why.SETTING_NOT_ALLOWED_IN_WORLD
        );
      }
      return island
        .map(x -> x.isAllowed(flag))
        .orElse(flag.isSetForWorld(loc.getWorld()));
    }
    ...
    if (
      user.hasPermission(
        getIWM().getPermissionPrefix(loc.getWorld()) + "mod.bypassprotect"
      ) ||
      user.hasPermission(
        getIWM().getPermissionPrefix(loc.getWorld()) +
        "mod.bypass." +
        flag.getID() +
        ".everywhere"
      )
    ...
    if (island.isPresent()) {
      ...
      noGo(
        ...
        flag,
        silent,
        island.get().isSpawn() ? "protection.spawn-protected" : "protection.protected"
      );
      return false;
    }
  ...
  private void report(
    ...
    if (
      ...
      user
        .getPlayer()
        .getMetadata(loc.getWorld().getName() + "_why_debug")
        .stream()
        .filter(p -> p.getOwningPlugin().equals(getPlugin()))
        .findFirst()
      ...
      String whyBypass =
        "Why: " + user.getName() + " " + flag.getID() + " - " + why.name();

      plugin.log(whyEvent);
      plugin.log(whyBypass);

      // See if there is a player that issued the debug
      String issuerUUID = user
        .getPlayer()
        .getMetadata(loc.getWorld().getName() + "_why_debug_issuer")
        .stream()
