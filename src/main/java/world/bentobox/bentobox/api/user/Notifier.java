...
public class Notifier {
  ...
  private final LoadingCache<User, Notification> notificationCache = CacheBuilder
    .newBuilder()
  ...
  public synchronized boolean notify(User user, String message) {
    try {
      ...
      if (
        now >= lastNotification.getTime() + (NOTIFICATION_DELAY * 1000) ||
        !message.equals(lastNotification.getMessage())
      ) {
