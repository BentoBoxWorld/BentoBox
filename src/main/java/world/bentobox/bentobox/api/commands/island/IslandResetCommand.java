  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (resetsLeft != -1) {
      ...
      if (resetsLeft == 0) {
        ...
        user.sendMessage(
          "commands.island.reset.resets-left",
          TextVariables.NUMBER,
          String.valueOf(resetsLeft)
        );
      }
    }
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (!args.isEmpty()) {
      String name = getPlugin()
        ...
        .validate(getAddon(), args.get(0).toLowerCase(java.util.Locale.ENGLISH));
      ...
      if (getPlugin().getSettings().isResetConfirmation()) {
        this.askConfirmation(
            user,
            user.getTranslation("commands.island.reset.confirmation"),
    ...
    island
      ...
      .forEach(
        memberUUID -> {
          ...
          Util.runCommands(
            member,
            getIWM().getOnLeaveCommands(island.getWorld()),
            "leave"
          );
          ...
          if (getSettings().isUseEconomy() && getIWM().isOnLeaveResetMoney(getWorld())) {
            getPlugin()
              .getVault()
              .ifPresent(vault -> vault.withdraw(member, vault.getBalance(member)));
          }
