...
public abstract class DefaultPlayerCommand extends CompositeCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (this.getIslands().getIsland(this.getWorld(), user.getUniqueId()) != null) {
      ...
      String command =
        this.<GameModeAddon>getAddon().getWorldSettings().getDefaultPlayerAction();
      ...
      String command =
        this.<GameModeAddon>getAddon().getWorldSettings().getDefaultNewPlayerAction();
      ...
      if (command != null && user.performCommand(label + " " + command)) {
        ...
        return this.getSubCommand("create")
          .map(
            createCmd ->
              createCmd.call(user, createCmd.getLabel(), Collections.emptyList())
          )
          .orElse(false);
      }
    }
