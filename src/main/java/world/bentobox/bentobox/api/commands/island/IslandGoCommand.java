  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (island.isReserved()) {
      ...
      getParent()
        ...
        .ifPresent(
          createCmd -> createCmd.call(user, createCmd.getLabel(), Collections.emptyList())
        );
      return false;
    }
    if (
      (
        getIWM().inWorld(user.getWorld()) &&
  ...
  public boolean execute(User user, String label, List<String> args) {
    if (!args.isEmpty() && NumberUtils.isDigits(args.get(0))) {
      ...
      if (homeValue > 1 && homeValue <= maxHomes) {
        this.delayCommand(
            user,
            () -> getIslands().homeTeleportAsync(getWorld(), user.getPlayer(), homeValue)
          );
        return true;
      }
    }
