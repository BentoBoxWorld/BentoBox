...
public class AdminSetSpawnPointCommand extends ConfirmableCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (
      ...
      this.askConfirmation(
          user,
          user.getTranslation("commands.admin.setspawnpoint.confirmation"),
  ...
  private void setSpawnPoint(User user, Island island) {
    island.setSpawnPoint(
      Objects.requireNonNull(user.getLocation().getWorld()).getEnvironment(),
      user.getLocation()
    );
        ...
        .forEach(
          player ->
            User
              .getInstance(player)
              .sendMessage(
                "commands.admin.setspawnpoint.island-spawnpoint-changed",
                "[user]",
                user.getName()
              )
        );
