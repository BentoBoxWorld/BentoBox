...
public class IslandSethomeCommand extends ConfirmableCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    // Check island
    if (
      !getPlugin().getIslands().hasIsland(getWorld(), user) &&
      !getPlugin().getIslands().inTeam(getWorld(), user.getUniqueId())
    ) {
      user.sendMessage("general.errors.no-island");
      return false;
    }
    if (
      !getPlugin().getIslands().locationIsOnIsland(user.getPlayer(), user.getLocation())
    ) {
    ...
    if (args.isEmpty()) {
      ...
      if (maxHomes > 1) {
        ...
        user.sendMessage(
          ...
          this.getPermissionPrefix() + "island.maxhomes.[number]"
        );
        return false;
      }
    }
  ...
  private boolean setHome(User user, int number) {
    ...
    if (getIWM().isNether(user.getWorld())) {
      ...
      if (
        getIWM()
          .getWorldSettings(user.getWorld())
          .isRequireConfirmationToSetHomeInNether()
      ) {
        askConfirmation(
          user,
          user.getTranslation("commands.island.sethome.nether.confirmation"),
      ...
      if (
        getIWM()
          .getWorldSettings(user.getWorld())
          .isRequireConfirmationToSetHomeInTheEnd()
      ) {
        askConfirmation(
          user,
          user.getTranslation("commands.island.sethome.the-end.confirmation"),
          () -> doSetHome(user, number)
        );
      } else {
        doSetHome(user, number);
