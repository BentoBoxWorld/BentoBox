...
public class AdminPurgeCommand extends CompositeCommand implements Listener {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    if (inPurge) {
      user.sendMessage(
        "commands.admin.purge.purge-in-progress",
        TextVariables.LABEL,
        this.getTopLabel()
      );
      return false;
  ...
  public boolean execute(User user, String label, List<String> args) {
    if (
      args.get(0).equalsIgnoreCase("confirm") && toBeConfirmed && this.user.equals(user)
    ) {
      removeIslands();
      return true;
    }
    ...
    try {
      ...
      user.sendMessage(
        "commands.admin.purge.purgable-islands",
  ...
  void removeIslands() {
    inPurge = true;
    user.sendMessage(
      "commands.admin.purge.see-console-for-status",
      TextVariables.LABEL,
      this.getTopLabel()
    );
    it = islands.iterator();
        ...
        .ifPresent(
          i -> {
            ...
            String percentage = String.format(
              "%.1f",
              (((float) count) / getPurgeableIslandsCount() * 100)
            );
            getPlugin()
              .log(
                count +
                " islands purged out of " +
                getPurgeableIslandsCount() +
                " (" +
                percentage +
                " %)"
              );
        ...
        i ->
          (
            (double) (
              ...
              Bukkit.getOfflinePlayer(i.getOwner()).getLastPlayed()
            ) /
            1000 /
            3600 /
            24
