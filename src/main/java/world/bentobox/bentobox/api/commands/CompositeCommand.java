...
public abstract class CompositeCommand
  extends Command
  implements PluginIdentifiableCommand, BentoBoxCommand {
  ...
  public boolean execute(CommandSender sender, String label, String[] args) {
    ...
    CommandEvent event = CommandEvent
      .builder()
      .setCommand(this)
      .setLabel(label)
      .setSender(sender)
  ...
  public boolean call(User user, String cmdLabel, List<String> cmdArgs) {
    ...
    if (
      ...
      !user.isOp() &&
      getPermission() != null &&
      !getPermission().isEmpty() &&
      !user.hasPermission(getPermission())
    ) {
      user.sendMessage(
  ...
  protected boolean hasSubCommand(String subCommand) {
    return (
      subCommands.containsKey(subCommand) || subCommandAliases.containsKey(subCommand)
    );
  }
  ...
  public void setPermission(String permission) {
    this.permission =
      (
        (permissionPrefix != null && !permissionPrefix.isEmpty()) ? permissionPrefix : ""
      ) +
      permission;
  }
   ...
   */
  @NonNull
  private List<String> getSubCommandLabels(
    @NonNull CommandSender sender,
    @NonNull CompositeCommand command
  ) {
    return command
      .getSubCommands()
      .values()
  ...
  protected boolean showHelp(CompositeCommand command, User user) {
    return command
      ...
      .map(
        helpCommand ->
          helpCommand.execute(user, helpCommand.getLabel(), new ArrayList<>())
      )
      .orElse(false);
  }
  ...
  public void setCooldown(String uniqueId, String targetUUID, int timeInSeconds) {
    cooldowns
      .computeIfAbsent(uniqueId, k -> new HashMap<>())
      .put(targetUUID, System.currentTimeMillis() + timeInSeconds * 1000);
  }
  ...
  public void setCooldown(UUID uniqueId, UUID targetUUID, int timeInSeconds) {
    cooldowns
      .computeIfAbsent(uniqueId.toString(), k -> new HashMap<>())
      .put(
  ...
  protected boolean checkCooldown(User user, String uniqueId, String targetUUID) {
    if (
      !cooldowns.containsKey(uniqueId) ||
      user.isOp() ||
    ...
    if (
      cooldowns.get(uniqueId).getOrDefault(targetUUID, 0L) -
    ...
    int timeToGo = (int) (
      (
        cooldowns.get(uniqueId).getOrDefault(targetUUID, 0L) - System.currentTimeMillis()
      ) /
      1000
    );
    user.sendMessage(
