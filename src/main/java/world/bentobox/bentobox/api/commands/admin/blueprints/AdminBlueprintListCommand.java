...
public class AdminBlueprintListCommand extends CompositeCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    List<String> blueprintList = Arrays
      .stream(Objects.requireNonNull(blueprints.list(blueprintFilter)))
      .map(
        name ->
          name.substring(0, name.length() - BlueprintsManager.BLUEPRINT_SUFFIX.length())
      )
