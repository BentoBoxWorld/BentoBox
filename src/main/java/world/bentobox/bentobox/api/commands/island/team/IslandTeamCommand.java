...
public class IslandTeamCommand extends CompositeCommand {
  ...
  private void showMembers(Island island, User user) {
    ...
    // Show header:
    user.sendMessage(
      "commands.island.team.info.header",
      "[max]",
      String.valueOf(inviteCommand.getMaxTeamSize(user)),
      "[total]",
      String.valueOf(island.getMemberSet().size()),
      "[online]",
      String.valueOf(onlineMembers.size())
    );
    ...
    onlineMembers =
      island
        ...
        .filter(
          uuid ->
            Util
              .getOnlinePlayerList(user)
              .contains(Bukkit.getOfflinePlayer(uuid).getName())
        )
        .collect(Collectors.toList());
    ...
    for (int rank : ranks) {
      ...
      if (!players.isEmpty()) {
        if (rank == RanksManager.OWNER_RANK) {
          ...
          user.sendMessage(
            "commands.island.team.info.rank-layout.owner",
            TextVariables.RANK,
            user.getTranslation(RanksManager.OWNER_RANK_REF)
          );
        } else {
          user.sendMessage(
            "commands.island.team.info.rank-layout.generic",
            TextVariables.RANK,
        ...
        for (UUID member : island.getMemberSet(rank, false)) {
          ...
          if (onlineMembers.contains(member)) {
            ...
            user.sendMessage(
              "commands.island.team.info.member-layout.online",
              TextVariables.NAME,
            ...
            if (duration.toMinutes() < 60L) {
              ...
              lastSeen =
                user.getTranslation(
                  reference,
                  TextVariables.NUMBER,
                  String.valueOf(duration.toHours()),
                  TextVariables.UNIT,
                  user.getTranslation("commands.island.team.info.last-seen.hours")
                );
            } else {
              lastSeen =
                user.getTranslation(
                  ...
                  TextVariables.UNIT,
                  user.getTranslation("commands.island.team.info.last-seen.days")
                );
            }

            if (island.getMemberSet(RanksManager.MEMBER_RANK, true).contains(member)) {
              user.sendMessage(
                "commands.island.team.info.member-layout.offline",
                TextVariables.NAME,
                offlineMember.getName(),
                "[last_seen]",
                lastSeen
              ...
              user.sendMessage(
                "commands.island.team.info.member-layout.offline-not-last-seen",
                TextVariables.NAME,
                offlineMember.getName()
              );
            }
