...
public class IslandTeamTrustCommand extends CompositeCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    // Check cooldown
    if (
      getSettings().getTrustCooldown() > 0 &&
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (island != null) {
      if (getPlugin().getSettings().isInviteConfirmation()) {
        ...
        target.sendMessage(
          "commands.island.team.trust.name-has-invited-you",
          TextVariables.NAME,
          user.getName()
        );
        target.sendMessage(
        ...
        if (
          island.getMemberSet(RanksManager.TRUSTED_RANK, false).size() >
          getMaxTrustSize(user)
        ) {
  ...
  public int getMaxTrustSize(User user) {
    return user.getPermissionValue(
      getPermissionPrefix() + "trust.maxsize",
      getIWM().getMaxTrustSize(getWorld())
    );
  }
}
