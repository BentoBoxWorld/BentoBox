...
public class IslandTeamCoopCommand extends CompositeCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (
      itc.isInvited(targetUUID) &&
      itc.getInviter(targetUUID).equals(user.getUniqueId()) &&
      itc.getInvite(targetUUID).getType().equals(Type.COOP)
    ) {
      // Prevent spam
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (island != null) {
      if (getPlugin().getSettings().isInviteConfirmation()) {
        ...
        target.sendMessage(
          "commands.island.team.coop.name-has-invited-you",
          TextVariables.NAME,
          user.getName()
        );
        target.sendMessage(
        ...
        if (
          island.getMemberSet(RanksManager.COOP_RANK, false).size() > getMaxCoopSize(user)
        ) {
  ...
  public int getMaxCoopSize(User user) {
    return user.getPermissionValue(
      getPermissionPrefix() + "coop.maxsize",
      getIWM().getMaxCoopSize(getWorld())
    );
  }
}
