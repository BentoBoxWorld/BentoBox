...
public class IslandExpelCommand extends CompositeCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (
      ...
      !getIslands().hasIsland(getWorld(), user)
    ) {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    Island island = getIslands().getIsland(getWorld(), user);
    // Fire event
    IslandBaseEvent expelEvent = IslandEvent
      .builder()
      .island(island)
    ...
    target.sendMessage(
      "commands.island.expel.player-expelled-you",
      TextVariables.NAME,
      user.getName()
    );
    island
    ...
    if (getIslands().hasIsland(getWorld(), target)) {
      ...
      getAddon()
        .logWarning("Expel: " + target.getName() + " had no island, so one was created");
      user.sendMessage(SUCCESS, TextVariables.NAME, target.getName());
      return true;
    }
    ...
    getAddon()
      .logError(
        "Expel: " + target.getName() + " had no island, and one could not be created"
      );
    user.sendMessage(CANNOT_EXPEL);
    return false;
  }
