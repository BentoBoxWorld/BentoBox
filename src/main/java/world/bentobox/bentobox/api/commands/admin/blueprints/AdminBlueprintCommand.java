...
public class AdminBlueprintCommand extends ConfirmableCommand {
  ...
  private static final Particle.DustOptions PARTICLE_DUST_OPTIONS = new Particle.DustOptions(
    Color.RED,
    1.0F
  );
  ...
  private void paintAxis(User user, BlueprintClipboard clipboard) {
    ...
    if (clipboard.getOrigin() != null) {
      user.spawnParticle(
        ...
        null,
        clipboard.getOrigin().getBlockX() + 0.5,
        clipboard.getOrigin().getBlockY() + 0.5,
        clipboard.getOrigin().getBlockZ() + 0.5
      );
    }
  }
