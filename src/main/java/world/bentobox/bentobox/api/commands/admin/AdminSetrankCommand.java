...
public class AdminSetrankCommand extends CompositeCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    rankValue =
      rm
        ...
        .entrySet()
        .stream()
        .filter(r -> user.getTranslation(r.getKey()).equalsIgnoreCase(args.get(1)))
        .findFirst()
    ...
    if (args.size() == 2) {
      ...
      if (!getPlugin().getIslands().hasIsland(getWorld(), ownerUUID)) {
        user.sendMessage(
          "general.errors.player-is-not-owner",
          TextVariables.NAME,
          args.get(2)
        );
        return false;
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    user.sendMessage(
      "commands.admin.setrank.rank-set",
      "[from]",
      user.getTranslation(rm.getRank(currentRank)),
      "[to]",
      user.getTranslation(rm.getRank(rankValue)),
      TextVariables.NAME,
      ownerName
    );
    return true;
    ...
    if (args.size() == 3) {
      return Optional.of(
        getPlugin()
          ...
          .map(entry -> user.getTranslation(entry.getKey()))
