...
public class IslandNearCommand extends CompositeCommand {
  private static final List<BlockFace> COMPASS_POINTS = Arrays.asList(
    BlockFace.NORTH,
    BlockFace.EAST,
    BlockFace.SOUTH,
    BlockFace.WEST
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (
      !getIslands().hasIsland(getWorld(), playerUUID) &&
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    for (BlockFace face : COMPASS_POINTS) {
      String name = getIslands()
        .getIslandAt(island.getCenter().getBlock().getRelative(face, dist).getLocation())
        .map(i -> getName(user, i))
        .orElse("");
      if (!name.isEmpty()) {
        noNeighbors = false;
        user.sendMessage(
          "commands.island.near.syntax",
          "[direction]",
          user.getTranslation(
            "commands.island.near." + face.name().toLowerCase(Locale.ENGLISH)
          ),
          TextVariables.NAME,
          name
        );
