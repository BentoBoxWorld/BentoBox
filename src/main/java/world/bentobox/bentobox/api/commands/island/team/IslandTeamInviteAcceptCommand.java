...
public class IslandTeamInviteAcceptCommand extends ConfirmableCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    String inviteUsage = getParent()
      .getSubCommand("invite")
      .map(CompositeCommand::getUsage)
      .orElse("");
    if (
      island == null ||
      island.getRank(prospectiveOwnerUUID) < island.getRankCommand(inviteUsage)
    ) {
      user.sendMessage("commands.island.team.invite.errors.invalid-invite");
    ...
    if (inviter != null) {
      ...
      if (island != null) {
        ...
        IslandEvent
          ...
          .build();
        inviter.sendMessage(
          "commands.island.team.trust.success",
          TextVariables.NAME,
          user.getName()
        );
        user.sendMessage(
          "commands.island.team.trust.you-are-trusted",
          TextVariables.NAME,
          inviter.getName()
        );
      }
    ...
    if (inviter != null) {
      ...
      if (island != null) {
        ...
        IslandEvent
          ...
          .build();
        inviter.sendMessage(
          "commands.island.team.coop.success",
          TextVariables.NAME,
          user.getName()
        );
        user.sendMessage(
          "commands.island.team.coop.you-are-a-coop-member",
          TextVariables.NAME,
          inviter.getName()
        );
      }
  ...
  private void acceptTeamInvite(User user, Invite invite) {
    ...
    getIslands()
      .homeTeleportAsync(getWorld(), user.getPlayer())
      .thenRun(
        () -> {
  ...
  private void cleanPlayer(User user) {
    if (
      getIWM().isOnLeaveResetEnderChest(getWorld()) ||
      getIWM().isOnJoinResetEnderChest(getWorld())
    ) {
      user.getPlayer().getEnderChest().clear();
    }
    if (
      getIWM().isOnLeaveResetInventory(getWorld()) ||
      getIWM().isOnJoinResetInventory(getWorld())
    ) {
      user.getPlayer().getInventory().clear();
    }
    if (
      getSettings().isUseEconomy() &&
      (
        getIWM().isOnLeaveResetMoney(getWorld()) ||
