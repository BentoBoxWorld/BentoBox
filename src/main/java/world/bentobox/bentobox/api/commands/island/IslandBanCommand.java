...
public class IslandBanCommand extends CompositeCommand {
  ...
  public boolean canExecute(User user, String label, List<String> args) {
    ...
    if (
      getSettings().getBanCooldown() > 0 &&
      checkCooldown(user, island.getUniqueId(), targetUUID.toString())
    ) {
      return false;
    }
    ...
    if (
      ...
      (
        target.isOnline() &&
        target.hasPermission(this.getPermissionPrefix() + "admin.noban")
      )
    ) {
  ...
  private boolean ban(@NonNull User issuer, User target) {
    ...
    int banLimit = issuer.getPermissionValue(
      getPermissionPrefix() + "ban.maxlimit",
      getIWM().getBanLimit(getWorld())
    );
    if (banLimit <= -1 || island.getBanned().size() < banLimit) {
      // Run the event
      IslandBaseEvent banEvent = IslandEvent
        .builder()
        .island(island)
      ...
      if (
        !banEvent.isCancelled() && island.ban(issuer.getUniqueId(), target.getUniqueId())
      ) {
        issuer.sendMessage(
          "commands.island.ban.player-banned",
          TextVariables.NAME,
          target.getName()
        );
        target.sendMessage(
          "commands.island.ban.owner-banned-you",
          TextVariables.NAME,
          issuer.getName()
        );
        ...
        if (
          target.isOnline() &&
          getIslands().hasIsland(getWorld(), target.getUniqueId()) &&
          island.onIsland(target.getLocation())
        ) {
          getIslands().homeTeleportAsync(getWorld(), target.getPlayer());
          island
            .getWorld()
            .playSound(target.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1F, 1F);
        }
        return true;
      }
    } else {
