...
public class AdminBlueprintSaveCommand extends ConfirmableCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (clipboard.isFull()) {
      ...
      File newFile = new File(
        ...
        fileName + BlueprintsManager.BLUEPRINT_SUFFIX
      );
      if (newFile.exists()) {
  ...
  private boolean hideAndSave(
    ...
    boolean result = new BlueprintClipboardManager(
      getPlugin(),
      parent.getBlueprintsFolder(),
      clipboard
    )
    ...
    if (result) {
      getPlugin()
        ...
        .addBlueprint(getAddon(), clipboard.getBlueprint());
    }
    return result;
  }
}
