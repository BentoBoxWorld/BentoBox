...
public class AdminTeamDisbandCommand extends CompositeCommand {
  ...
  public boolean execute(User user, String label, List<String> args) {
    ...
    if (!getIslands().getOwner(getWorld(), targetUUID).equals(targetUUID)) {
      user.sendMessage(
        "commands.admin.team.disband.use-disband-owner",
        "[owner]",
        getPlayers().getName(getIslands().getOwner(getWorld(), targetUUID))
      );
      return false;
    }
    // Disband team
    ...
    getIslands()
      .getMembers(getWorld(), targetUUID)
      .forEach(
        m -> {
    ...
    user.sendMessage(
      "commands.admin.team.disband.success",
      TextVariables.NAME,
      args.get(0)
    );
    return true;
