...
/**
 * This is template object for the panel reader. It contains data that can exist
 * in the panel. PanelBuilder will use this to build panel.
 *
 ...
 * @param forcedRows the array of boolean that indicate which rows must be force
 *                   loaded.
 * @param content    The 2D array of ItemTemplateRecords
 *
 * @since 1.17.3
 */
public record PanelTemplateRecord(Panel.Type type, @Nullable String title, @Nullable TemplateItem border,
        @Nullable TemplateItem background, boolean[] forcedRows, @NonNull ItemTemplateRecord[][] content) {
    ...
    public String toString() {
        return "PanelTemplateRecord {type=" + type + ", title=" + title + ", border=" + border + ", background="
                + background + ", forcedRows=" + Arrays.toString(forcedRows) + ", content=" + Arrays.toString(content)
                + "}";
    }
