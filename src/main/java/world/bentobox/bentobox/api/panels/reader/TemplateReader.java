...
/**
 * This class manages Template file reading, creating PanelTemplateRecord object
 * and storing it internally. This class just reads and returns given panel
 * template. It does not create a functional panel.
 *
 * @since 1.17.3
 */
public class TemplateReader {
     ...
     * not have public constructors. Java adds an implicit public constructor to
     * every class which does not define at least one explicitly. Hence, at least
     * one non-public constructor should be defined.
     */
     ...
     * @since 1.20.0
     */
    public static PanelTemplateRecord readTemplatePanel(@NonNull String panelName, @NonNull String templateName,
            @NonNull File panelLocation) {
    ...
    private static PanelTemplateRecord readPanelTemplate(@Nullable ConfigurationSection configurationSection) {
        ...
        Panel.Type type = Enums.getIfPresent(Panel.Type.class, configurationSection.getString(TYPE, "INVENTORY"))
        ...
        if (configurationSection.isConfigurationSection(BORDER)) {
            ...
            if (borderSection != null) {
                borderItem = new PanelTemplateRecord.TemplateItem(
                        ...
                        borderSection.getString(TITLE, null), borderSection.getString(DESCRIPTION, null));
            }
        ...
        if (configurationSection.isConfigurationSection(BACKGROUND)) {
            ...
            if (backgroundSection != null) {
                backgroundItem = new PanelTemplateRecord.TemplateItem(
                        ...
                        backgroundSection.getString(TITLE, null), backgroundSection.getString(DESCRIPTION, null));
            }
        ...
        if (reusable != null) {
            ...
            reusable.getKeys(false).forEach(
                    key -> readPanelItemTemplate(reusable.getConfigurationSection(key), key, panelItemDataMap));
        }
     ...
     * 
     * @param section Section that should contain all information about the panel
     *                item template.
     ...
     */
    @Nullable
    private static ItemTemplateRecord readPanelItemTemplate(@Nullable ConfigurationSection section, String itemKey,
        ...
        ItemTemplateRecord itemRecord = new ItemTemplateRecord(ItemParser.parse(section.getString(ICON)),
                section.getString(TITLE, null), section.getString(DESCRIPTION, null), fallback);
        ...
        if (section.isConfigurationSection(ACTIONS)) {
                ...
                actionSection.getKeys(false).forEach(actionKey -> {
                    ...
                    if (clickType != null) {
                        ...
                        if (actionDataSection != null) {
                            ItemTemplateRecord.ActionRecords actionData = new ItemTemplateRecord.ActionRecords(
                                    clickType, actionDataSection.getString(TYPE), actionDataSection.getString(CONTENT),
                        ...
                        if (actionDataSection != null && actionDataSection.contains(CLICK_TYPE)) {
                            clickType = Enums
                                    .getIfPresent(ClickType.class,
                                            actionDataSection.getString(CLICK_TYPE, "UNKNOWN").toUpperCase())
                                    .or(ClickType.UNKNOWN);

                            ItemTemplateRecord.ActionRecords actionData = new ItemTemplateRecord.ActionRecords(
                                    clickType, actionKey, actionDataSection.getString(CONTENT),
                                    actionDataSection.getString(TOOLTIP));
                            itemRecord.addAction(actionData);
                        }
                    }
            ...
            if (!actionList.isEmpty()) {
                actionList.forEach(valueMap -> {
                    ClickType clickType = Enums
                            .getIfPresent(ClickType.class, String.valueOf(valueMap.get(CLICK_TYPE)).toUpperCase())
                            .orNull();

                    if (clickType != null) {
                        ItemTemplateRecord.ActionRecords actionData = new ItemTemplateRecord.ActionRecords(clickType,
    ...
    /**
     * This map contains already read panels and their location. This improves
     * performance for GUI opening, with a some memory usage.
     */
