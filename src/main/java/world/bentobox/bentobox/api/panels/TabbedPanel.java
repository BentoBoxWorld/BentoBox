...
public class TabbedPanel extends Panel implements PanelListener {
  ...
  public void openPanel(int activeTab, int page) {
    ...
    if (page < 0) {
      // Request to open a non-existent tab
      throw new InvalidParameterException(
        "Attempt to open a tab in a tabbed panel to a negative page! " + page
      );
    }
    ...
    if (tpb.isHideIfEmpty()) {
      tpb
        ...
        .values()
        .removeIf(
          t -> !t.equals(tab) && t.getPanelItems().stream().noneMatch(Objects::nonNull)
        );
    }
    ...
    if (tpb.getTabs().containsKey(activeTab)) {
      ...
      panelItems
        ...
        .forEach(i -> items.put(items.lastKey() + 1, i));
      ...
      if (
        (page + 1) * ITEMS_PER_PAGE < panelItems.stream().filter(Objects::nonNull).count()
      ) {
        // Next page icon
        items.put(
          52,
  ...
  public void onInventoryClick(User user, InventoryClickEvent event) {
    ...
    if (
      event.isLeftClick() &&
      tpb.getTabs().containsKey(event.getRawSlot()) &&
      (
        tpb.getTabs().get(event.getRawSlot()).getPermission().isEmpty() ||
        tpb
          .getUser()
          .hasPermission(tpb.getTabs().get(event.getRawSlot()).getPermission()) ||
        tpb.getUser().isOp()
      )
    ) {
