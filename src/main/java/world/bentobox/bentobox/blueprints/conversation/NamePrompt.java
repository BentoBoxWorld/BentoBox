...
public class NamePrompt extends StringPrompt {
  ...
  public NamePrompt(
    ...
    @Nullable Blueprint bp,
    @Nullable BlueprintBundle bb
  ) {
    this.addon = addon;
    this.bp = bp;
    this.bb = bb;
  ...
  public Prompt acceptInput(ConversationContext context, String input) {
    ...
    if (bb == null || !bb.getUniqueId().equals(BlueprintsManager.DEFAULT_BUNDLE_NAME)) {
      ...
      StringBuilder uniqueId = new StringBuilder(
        ChatColor.stripColor(input).toLowerCase(Locale.ENGLISH).replace(" ", "_")
      );
      ...
      if (max == 32) {
        context
          ...
          .sendRawMessage(
            user.getTranslation(
              "commands.admin.blueprint.management.name.pick-a-unique-name"
            )
          );
        return this;
      }
