...
public class MySQLDatabaseHandlerTest {
  ...
  public void testSaveObjectNotDataObject() {
    ...
    MySQLDatabaseHandler<List> h = new MySQLDatabaseHandler<List>(
      plugin,
      List.class,
      dbConn
    );
    ...
    verify(plugin)
      .logError(
        eq("This class is not a DataObject: java.util.Collections$SingletonList")
      );
  }
  ...
  public void testObjectExistsPrefix() throws SQLException {
    ...
    verify(connection)
      .prepareStatement(
        "CREATE TABLE IF NOT EXISTS `aIslands` (json JSON, uniqueId VARCHAR(255) GENERATED ALWAYS AS (json->\"$.uniqueId\"), UNIQUE INDEX i (uniqueId) ) ENGINE = INNODB"
      );
    verify(ps).executeQuery();
    verify(ps).setString(1, "\"hello\"");
  }
  ...
  public void testMySQLDatabaseHandlerSchemaFail() throws SQLException {
    ...
    verify(plugin)
      .logError(
        "Problem trying to create schema for data object world.bentobox.bentobox.database.objects.Island oh no!"
      );
  }
}
