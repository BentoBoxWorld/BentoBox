...
public class ItemParserTest {
  ...
  public void setUp() throws Exception {
    ...
    when(itemFactory.getItemMeta(Mockito.any()))
      .thenAnswer(
        (Answer<ItemMeta>) invocation -> {
      ...
      for (ex e : ex.values()) {
        for (ty t : ty.values()) {
          for (int up = 1; up < 2; up++) {
            ...
            String req =
              "POTION:" + type.name() + ":" + up + ":" + e.name() + ":" + t.name() + ":3";
            ...
            switch (t) {
              case LINGER:
                ...
                PotionData data = new PotionData(type, isExtended, isUpgraded);
                Mockito
                  .verify(potionMeta, Mockito.times(3))
                  .setBasePotionData(Mockito.eq(data));
                break;
              case NO_SPLASH:
                assertEquals(Material.POTION, result.getType());
              ...
              case SPLASH:
                ...
                data = new PotionData(type, isExtended, isUpgraded);
                Mockito
                  .verify(potionMeta, Mockito.times(2))
                  .setBasePotionData(Mockito.eq(data));
                break;
              default:
                break;
