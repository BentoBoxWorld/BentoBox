...
public class IslandBanCommandTest {
  ...
  public void testTabComplete() {
    ...
    when(island.isBanned(any(UUID.class)))
      .thenAnswer(
        (Answer<Boolean>) invocation ->
          banned.contains(invocation.getArgument(0, UUID.class))
      );
    // Create the names
    when(pm.getName(any(UUID.class)))
      .then(
        (Answer<String>) invocation ->
          online.getOrDefault(invocation.getArgument(0, UUID.class), "tastybento")
      );
    ...
    when(player.canSee(any(Player.class)))
      .thenAnswer(
        (Answer<Boolean>) invocation -> {
