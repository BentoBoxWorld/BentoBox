...
public class IslandTeamInviteCommandTest {
  ...
  public void testCanExecuteCoolDownActive() {
    ...
    assertFalse(
      itl.canExecute(user, itl.getLabel(), Collections.singletonList("target"))
    );
    verify(user)
      .sendMessage(
        eq("general.errors.you-must-wait"),
        eq(TextVariables.NUMBER),
        anyString()
      );
  }
  ...
  public void testCanExecuteLowRank() {
    ...
    verify(user)
      .sendMessage(
        eq("general.errors.insufficient-rank"),
        eq(TextVariables.RANK),
        eq("ranks.member")
      );
  }
  ...
  public void testCanExecuteNoTarget() {
    ...
    verify(user)
      .sendMessage(
        eq("commands.help.header"),
        eq(TextVariables.LABEL),
        eq("commands.help.console")
      );
  }
  ...
  public void testCanExecuteSamePlayer() {
    assertFalse(
      itl.canExecute(user, itl.getLabel(), Collections.singletonList("tastybento"))
    );
  ...
  public void testCanExecuteUnknownPlayer() {
    ...
    verify(user)
      .sendMessage(
        eq("general.errors.unknown-player"),
        eq(TextVariables.NAME),
        eq("target")
      );
  }
  ...
  public void testExecuteSuccessTargetHasIsland() {
    ...
    verify(user)
      .sendMessage(
        eq("commands.island.team.invite.invitation-sent"),
        eq(TextVariables.NAME),
        eq("target")
      );
    verify(target)
      .sendMessage(
        eq("commands.island.team.invite.name-has-invited-you"),
        eq(TextVariables.NAME),
        eq("tastybento")
      );
    verify(target)
      .sendMessage(
        eq("commands.island.team.invite.to-accept-or-reject"),
        eq(TextVariables.LABEL),
        eq("island")
      );
    verify(target)
      .sendMessage(eq("commands.island.team.invite.you-will-lose-your-island"));
  }
