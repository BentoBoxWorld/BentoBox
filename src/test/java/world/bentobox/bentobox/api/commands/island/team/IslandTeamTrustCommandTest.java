...
public class IslandTeamTrustCommandTest {
  ...
  public void setUp() throws Exception {
    ...
    when(island.getMemberSet(anyInt(), any(Boolean.class)))
      .thenReturn(ImmutableSet.of(uuid));
  ...
  public void testExecuteNullIsland() {
    ...
    assertFalse(
      itl.execute(user, itl.getLabel(), Collections.singletonList("tastybento"))
    );
  ...
  public void testExecuteSuccessNoConfirmation() {
    ...
    verify(user)
      .sendMessage(
        eq("commands.island.team.trust.success"),
  ...
  public void testExecuteSuccessConfirmation() {
    ...
    verify(targetPlayer)
      .sendMessage(eq("commands.island.team.trust.name-has-invited-you"));
    verify(targetPlayer)
      .sendMessage(eq("commands.island.team.invite.to-accept-or-reject"));
