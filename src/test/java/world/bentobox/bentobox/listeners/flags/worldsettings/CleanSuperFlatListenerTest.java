...
public class CleanSuperFlatListenerTest {
  ...
  public void setUp() throws Exception {
    ...
    // Super flat!
    when(block.getType())
      .thenReturn(Material.BEDROCK, Material.DIRT, Material.DIRT, Material.GRASS_BLOCK);
    when(chunk.getBlock(Mockito.anyInt(), Mockito.anyInt(), Mockito.anyInt()))
      .thenReturn(block);
    ...
    when(
        cg.generateChunkData(
          any(World.class),
          any(Random.class),
          anyInt(),
          anyInt(),
          any(MyBiomeGrid.class)
        )
      )
      .thenReturn(cd);
  ...
  public void testOnChunkLoadNotBedrockNoFlsg() {
    ...
    verify(scheduler, never())
      .runTaskTimer(any(), any(Runnable.class), Mockito.eq(0L), Mockito.eq(1L));
  }
